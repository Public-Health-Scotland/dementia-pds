...
pds_all_totals <- bind_rows(
  pds_all %>% filter(age_grp_2 != "Unknown") %>% group_by(geog, fy) %>% summarise(total_referrals = sum(referrals))#,
...
pds_ijb_totals <- bind_rows(
    pds_ijb %>% filter(age_grp_2 != "Unknown") %>% group_by(geog, fy) %>% summarise(total_referrals = sum(referrals))#,
...
pds_hb_totals <- bind_rows(
    pds_hb %>% filter(age_grp_2 != "Unknown") %>% group_by(geog, fy) %>% summarise(total_referrals = sum(referrals))#,
...
pds_scot_totals <- bind_rows(
    pds_scot %>% filter(age_grp_2 != "Unknown") %>% group_by(geog, fy) %>% summarise(total_referrals = sum(referrals))#,
...
pop_data_trends <- pop_data %>%  filter(age_grp == "All", sex == "All", age_grp_2 == "All") %>% select(geog, year, pop_est)
...
trends_plot <- plot_trend(pds_scot_totals, total_referrals, x ="Financial Year of Diagnosis", y = "Number")
...
<h2 style="margin-left: 50px; margin-top: 0px; margin-bottom: 0px"> Number of Individuals Diagnosed and Referred for PDS per 10,000 Population by Health Board </h2>
```{r plot-hb-pop}
pds_scot_pop_data <- SharedData$new(pds_scot_rates,
                                    key = ~fy, group = "grouphb")
 
 hb_rates_plot <- plot_bar_pop_rate(pds_hb_pop_data, pds_scot_pop_data)
...
```{r plot-ijb-pop}
pds_scot_pop_data_2 <- SharedData$new(pds_scot_rates,
                                    key = ~fy, group = "groupijb")

ijb_rates_plot <- plot_bar_pop_rate(pds_ijb_pop_data, pds_scot_pop_data_2)
...
bind_rows(pds_scot_totals, pds_hb_totals) %>% filter(fy != current_fy),
...
<h2 style="margin-left: 50px; margin-top: 0px; margin-bottom: 0px"> Percentage of people estimated to be newly diagnosed with dementia who were referred for post-diagnostic support </h2>
```{r trend-plot-exp}
...
<h2 style="margin-left: 50px; margin-top: 0px; margin-bottom: 0px"> Percentage of people referred for dementia PDS who received a minimum of one year’s support </h2>
```{r plot-med-ldp}
