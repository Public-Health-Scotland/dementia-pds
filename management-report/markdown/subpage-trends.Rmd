...
rates <- create_rates_table_age_sex(ldp_df, population_df, ESP13, 
                                age_cutoff = 65)

rates_age <- create_rates_table_age_sex(ldp_df, population_df, ESP13, 
                                age_cutoff = 65, 
                                grouping = c("age_grp_2"))

rates_sex <- create_rates_table_age_sex(ldp_df, population_df, ESP13, 
                                age_cutoff = 65, 
                                grouping = c("sex"))

rates_simd <- create_rates_table_age_sex(ldp_df, population_df, ESP13, 
                                age_cutoff = 65, 
                                grouping = c("simd"))
```
...
trends_plot <- plot_trend(rates %>% filter(name == "scot"), total_referrals, x ="Financial Year of Diagnosis", y = "Number Referred", measure_text = "Referrals: ", Scotland = TRUE)
...
rates %>% 
  ...
  select(geog, fy, total_referrals) %>% 
  ...
  add_header_above(c(" " = 1, "Financial Year of Diagnosis" = length(all_years))) %>%
...
```{r dropdown-ijb}
pds_trends_data_ijb <- SharedData$new(pds_ijb_totals %>% filter(geog != "Unknown"))
...
```{r dropdown-pop}
pds_trends_pop_data <- SharedData$new(pds_all_rates %>% filter(geog != "Unknown"))
...
exp %>% select(-health_board) %>% rename(geog = health_board_label)) %>% 
  mutate(exp_rate = round_half_up(100*total_referrals/diagnoses,1))
...
```{r dropdown-met-ldp}
pds_met_rates <-
  pds_all %>% filter(ldp != "ongoing", fy %in% full_years) %>% filter(geog != "Unknown") %>% 
  group_by(geog, fy, ldp) %>% 
  summarise(referrals = sum(referrals)) %>% pivot_wider(names_from = ldp, values_from = referrals, values_fill = 0) %>% 
  mutate(perc_met = round_half_up(100*(complete + exempt)/(complete + exempt + fail), 1))
...
pds_met_rates %>% select(geog, fy, perc_met) %>% 
            ...
            mutate(across(where(is.numeric), ~if_else(is.na(.), "\\-", paste0(.,"%")))) %>%
            rename(`Integration Authority Area/Health Board` = geog) %>%
            kable(align = c("l", rep("r", length(full_years)))) %>%
            kable_styling(full_width = TRUE, c("striped","hover")) %>%
            column_spec(1, bold = TRUE) %>%
