...
pds_scot <- pds %>% group_by(health_board = "Scotland", ijb = "Scotland", fy, month, ldp, age_grp_2, age_grp, simd) %>% 
  summarise(referrals = sum(referrals), .groups = "drop")

pds_hb <- pds %>% group_by(health_board, fy, month, ldp, age_grp_2, age_grp, simd) %>% 
...
pop_data_trends <- pop_data %>%  filter(age_grp == "All", age_grp_2 == "All", sex == "All") %>% select(geog, year, pop_est)
...
trends_plot <- plot_trend(pds_scot, total_referrals, x ="Financial Year of Diagnosis", y = "Number")
