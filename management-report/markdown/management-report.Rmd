...
<!-- ADDITIONAL ANALYSIS -->
`r knitr::knit_child("subpage-additional-analysis.Rmd")`
...
<h2 style="margin-left: 10px; margin-top: 0px; margin-bottom: 0px"> Percentage of records with missing/unknown entry (NA/codes 98/99) for Ethnic Group</h2>
          ...
          DT::datatable(ethnic_comp_data, 
              ...
              options = list(pageLength = 46,
                             ...
                             columnDefs = list(list(visible = F, targets = 0),
                                              list(className = 'dt-right', targets = 2:length(all_years)+2)
                             ))) %>%
...
<h2 style="margin-left: 10px; margin-top: 0px; margin-bottom: 0px"> Percentage of records with missing/unknown entry (NA/codes 98/99) for Accommodation Type</h2>
...
bscols(widths = 12,
       div(style = css(width="98%", height="650px", margin="15px"),
        ...
        DT::formatStyle(0, target = "row",
              fontWeight = DT::styleEqual(dim(accomm_comp_data)[1],"bold"))
                        )
       )
...
<h2 style="margin-left: 10px; margin-top: 0px; margin-bottom: 0px"> Percentage of records with missing/unknown entry (NA/codes 98/99) for Carer Support</h2>

```{r carer-table}
carer_comp_data <- comp_data %>%
  filter(field_name == "carers_support") %>% select(geog, fy, perc_of_records_missing_not_known) %>% 
  mutate(perc_of_records_missing_not_known = paste0(perc_of_records_missing_not_known, "%")) %>% 
  pivot_wider(names_from = fy, values_from = perc_of_records_missing_not_known) %>% 
  rename(`Health Board/Integration Authority` = geog) %>%
  slice(2:46,1)
...
<h2 style="margin-left: 10px; margin-top: 0px; margin-bottom: 0px"> Percentage of records with missing/unknown entry (NA/codes 98/99) and Percentage of records Yet to be determined (YTBD) for Subtype of Dementia</h2>
...
headers <- htmltools::withTags(table(
  ...
  thead(
    tr(
      th(colspan = 1, '    '),
      th(rowspan = 2, "Health Board/Integration Authority Area"),
    ...
    tr(
      lapply(c(" ",rep(c("NA/98/99", "YTBD"), 10)), th)
    )
  )
))
...
<h2 style="margin-left: 10px; margin-top: 0px; margin-bottom: 0px"> Percentage of records with missing/unknown entry (NA/codes 98/99) and Percentage of records Yet to be determined (YTBD) for Clinical Impression of Stage of Illness</h2>
...
<h2 style="margin-left: 10px; margin-top: 0px; margin-bottom: 0px"> Percentage of records with missing/unknown entry (NA/codes 98/99) and Percentage of records Yet to be determined (YTBD) for Model of Care</h2>
...
#           "2024/25" = 2
#       ))%>%
#   scroll_box(height = "670px", width = "auto", fixed_thead = T)
