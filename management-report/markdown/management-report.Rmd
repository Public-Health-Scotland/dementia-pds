...
Infomation on the methodology used to calculate these figures and the limitations of this are available in the report.
...
* Where referral, allocation, first contact or termination date pre-date diagnosis date, these are fixed to diagnosis date.
...
Records that are removed before analysis include those with a diagnosis date outwith the reporting period (i.e. before `r format(start_date, "%d %B %Y")` or after `r format(end_date, "%d %B %Y")`), and records with no diagnosis date provided.

The table below shows the percentage of records which contain one or more errors.
