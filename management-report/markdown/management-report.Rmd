...
# LDP Data
ldp_df <- read_rds(get_mi_data_path("ldp_data", ext = "rds", test_output = test_output)) %>%
  mutate(
...
exp_df <- read_csv(get_exp_diagnoses_path()) %>%
  rename(geog = health_board_label) %>% # Rename health board column to geog to match PDS and population data
  select(-health_board) # Remove health board code column
...
# Rates
################################################################################.
standardised_rates <- full_join(
  create_rates_table(ldp_df, population_df, ESP13_df, standardisation = "age-sex", 
                     age_cutoff = 65, demographic = c()), 
  create_rates_table(ldp_df, population_df, ESP13_df, standardisation = "age", 
                     age_cutoff = 65, demographic = c()))

standardised_rates_age <- full_join(
  create_rates_table(ldp_df, population_df, ESP13_df, standardisation = "age-sex", 
                     age_cutoff = 65, demographic = c("age_grp_2")), 
  create_rates_table(ldp_df, population_df, ESP13_df, standardisation = "age", 
                     age_cutoff = 65, demographic = c("age_grp_2")))

standardised_rates_sex <- full_join(
  create_rates_table(ldp_df, population_df, ESP13_df, standardisation = "age-sex", 
                     age_cutoff = 65, demographic = c("sex")), 
  create_rates_table(ldp_df, population_df, ESP13_df, standardisation = "age", 
                     age_cutoff = 65, demographic = c("sex")))

standardised_rates_simd <- full_join(
  create_rates_table(ldp_df, population_df, ESP13_df, standardisation = "age-sex", 
                     age_cutoff = 65, demographic = c("simd")), 
  create_rates_table(ldp_df, population_df, ESP13_df, standardisation = "age", 
                     age_cutoff = 65, demographic = c("simd")))
...
# Other Measures
################################################################################.
measures <- create_measures_table(ldp_df, exp_df, demographic = c())
measures_age <- create_measures_table(ldp_df, exp_df, demographic = c("age_grp_2"))
measures_sex <- create_measures_table(ldp_df, exp_df, demographic = c("sex"))
measures_simd <- create_measures_table(ldp_df, exp_df, demographic = c("simd"))
```
