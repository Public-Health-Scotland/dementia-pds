...
################################################################################.
# Load setup, packages and functions ----
################################################################################.
...
library(htmltools)
detach("package:tidylog", unload = TRUE) # Remove tidylog
...
# fy, qt and finalised_years are defined in 00_setup-environment
...
last_full_year <- if_else(
  qt == 4, 
  current_fy, 
  paste0(
    as.numeric(substr(fy,1,4)) - 1, 
    "/", 
    as.numeric(substr(fy,3,4))
  )
)

all_years <- unique(standardised_rates$fy)

full_years <- ifelse(qt == 4, all_years, all_years[-length(all_years)])
...
comp_data <- read_rds(get_mi_data_path(
  type = "comp_data", 
  ext = "rds", 
  fy = fy,
  qt = qt,
...
age_unknown_all_years <- pds_scot %>% 
  filter(age_grp == "Unknown")

age_unknown <- sum(age_unknown_all_years$referrals)

sex_unknown_all_years <- pds_scot %>% 
  filter(sex != "01 Male", sex != "02 Female")

sex_unknown <- sum(sex_unknown_all_years$referrals)

simd_unknown_all_years <- pds_scot %>% 
  filter(simd == "Unknown")
...
Data are submitted to PHS by health boards on a quarterly basis. Each health board provides updated information for all individuals referred for post diagnostic support with a diagnosis date for years that are still considered provisional, currently from `r format(ymd(paste0(substr(provisional_year, 1,4), "-04-01")), "%d %B %Y")` onwards. Therefore, data for diagnoses from this date onwards are refreshed in each management report and are based on the most recently submitted data. Data for provisional years may contain errors due to errors in the data submitted. These are  identified and reported to submitters as part of the PHS data assurance process and corrected data is requested. Data for provisional years can be revised until they are considered final. **Data for diagnoses prior to `r format(ymd(paste0(substr(provisional_year, 1,4), "-04-01")), "%d %B %Y")` are now final and will not change in future versions of the report.**
...
**NHS Grampian** - Due to changes in service provider the figures for Aberdeen City in *2019/20* and *2020/21* have been affected. In *2025/26 Q4* submission the numbers reported for  *2023/24*, *2024/25* and *2025/26* are lower than those reported than in previous quarters.  PHS are working with the Board to investigate and resolve this.

**NHS Tayside** – Due to local IT system issues the figures for *2018/19* and *2019/20* have been affected. PHS have identified issues with the data submitted by NHS Tayside for *2023/24*, *2024/25* and *2025/26*, which has resulted in some records showing as incorrectly having met the standard. NHS Tayside has informed PHS that these data issues will be rectified once they have completed the migration to a new IT system. Due to the IT migration NHS Tayside have been unable to submit data from all areas for *2025/26 Q3 and Q4*.
...
*For the rates calculations, for financial year 2025/26 the Mid-2024 populations are used as this is the latest available estimate.*  <!-- UPDATE when new population estimates are available / when new financial year begins -->
...
```{r results="asis"}
cat(sprintf('<script>var pds_year = "%s"; var ldp_year = "%s";</script>', 
            current_fy, last_full_year))
```
...
function filter_default() {
  ...
  function setFilter(id, value) {
    try {
      ...
      if(value) el.setValue(value, false);
      el.removeOption("");
    } catch(e) {
      console.log("Failed to set filter: " + id, e);
    }
  }

  // all referrals - trends and rates
  setFilter("trend_filter_hb", "NHS Ayrshire & Arran");
  setFilter("trend_filter_ijb", "Aberdeen City");
  setFilter("trend_filter_pop", "Scotland");
  setFilter("trend_filter_year_hb", pds_year);
  setFilter("trend_filter_rate_hb", "old_rate");
  setFilter("trend_filter_year_ijb", pds_year);
  setFilter("trend_filter_rate_ijb", "old_rate");
  setFilter("trend_filter_geog_trend", "Scotland");
  setFilter("trend_filter_rate_trend", "old_rate");
  setFilter("trend_filter_outcomes_exp", "Scotland");
  setFilter("trend_filter_outcomes_met", "Scotland");

  // demographics
  setFilter("filter_ijb_age_pop", pds_year);
  setFilter("filter_ijb_age_pop_geog", null);
  setFilter("filter_hb_age_pop", pds_year);
  setFilter("filter_ijb_age_ldp_geog", null);
  setFilter("filter_trend_age_pop", "Scotland");
  setFilter("filter_ijb_age_ldp", ldp_year);
  setFilter("filter_hb_age_ldp", ldp_year);
  setFilter("filter_ldp_age", "Scotland");
  setFilter("filter_trend_sex_pop", "Scotland");
  setFilter("filter_hb_sex_pop", pds_year);
  setFilter("filter_ijb_sex_pop", pds_year);
  setFilter("filter_ijb_sex_ldp", ldp_year);
  setFilter("filter_hb_sex_ldp", ldp_year);
  setFilter("filter_ldp_sex", "Scotland");
  setFilter("filter_trend_simd_pop", "Scotland");
  setFilter("filter_hb_simd_pop", pds_year);
  setFilter("filter_ijb_simd_pop", pds_year);
  setFilter("filter_ijb_simd_pop_geog", null);
  setFilter("filter_ijb_simd_ldp_geog", null);
  setFilter("filter_ldp_simd", "Scotland");
  setFilter("filter_hb_simd_ldp", ldp_year);
  setFilter("filter_ijb_simd_ldp", ldp_year);
  setFilter("filter_ldp_simd_bar", "Scotland");

  // pathways
  setFilter("not_met_filter_year", pds_year);
  setFilter("ongoing_filter_year", pds_year);
  setFilter("exempt_filter_year", pds_year);
  setFilter("wait_filter_all_year", pds_year);
  setFilter("filter_median", pds_year);
  setFilter("filter_median_hb", pds_year);
  setFilter("trend_filter_wait", "Scotland");
  setFilter("filter_uptake_hb_year", pds_year);
  setFilter("filter_uptake_ijb_year", pds_year);
  setFilter("no_contact_filter_year_hb", pds_year);
  setFilter("no_contact_filter_year_ijb", pds_year);
