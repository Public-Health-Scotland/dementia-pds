...
`r ifelse(complete_year == TRUE, glue("A total of <span style='color:CornflowerBlue'>**{ref}**</span> referrals were made to post-diagnostic support. This is divided by <span style='color:CornflowerBlue'>**{exp}**</span>, the estimated number of people newly diagnosed with dementia."), "")`
...
`r ifelse(complete_year == TRUE, glue("<span style='color:CornflowerBlue'>**{num}**</span> of referrals either met or were exempt from the LDP standard. This is divided by <span style='color:CornflowerBlue'>**{den}**</span>, the total number of referrals (excluding those whose support is ongoing)."), "")`
...
subpage_data %>%
  ...
  select(ldp, c(subpage_data$health_board, sort(unique(subpage_data$ijb)))) %>%
  arrange(match(ldp, c("Standard Met", "Standard Not Met", "PDS Ongoing", "Exempt", "Total"))) %>%
  kable(col.names = c("", unique(subpage_data$health_board), sort(unique(subpage_data$ijb)))) %>%
