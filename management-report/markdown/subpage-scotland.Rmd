...
est_date5 <- dmy(01042025)

  est_text <-  
    case_when(sel_fy ==
extract_fin_year(est_date1) | sel_fy ==
extract_fin_year(est_date2) | sel_fy == extract_fin_year(est_date3) | sel_fy == extract_fin_year(est_date4) | sel_fy == extract_fin_year(est_date5) ~
      paste0("",
             "**Please note:** ",
             "In December 2016, the Scottish Government published a report: ",
             "[Estimated and Projected Diagnosis Rates for Dementia in Scotland: 2014-2020](https://www.gov.scot/publications/estimated-projected-diagnosis-rates-dementia-scotland-2014-2020/){target='_blank'}. ", 
             ...
             "Scotland (NRS) Mid-2021 (rebased), Mid-2022 and Mid-2023 Population Estimates to obtain the incidence estimates."),
      TRUE ~ paste0(""))
...
# Data quality notes - Aberdeen City
  ...
  tay_date5 <- dmy(01042025)
  
  tay_text <-  
    case_when(sel_fy == extract_fin_year(tay_date1) |
              sel_fy == extract_fin_year(tay_date2) ~
        paste0(" ", 
           "**Please note:** ",
           ...
           "allowed NHS Tayside to improve on data reporting from 2019 onwards.*"),
              sel_fy == extract_fin_year(tay_date3)|
              sel_fy == extract_fin_year(tay_date4)|
              sel_fy == extract_fin_year(tay_date5)~
        paste0(" ", 
           "**Please note:** ",
              "*PHS have identified issues with the data submitted by NHS Tayside for 2023/24, ", 
              "2024/25 and 2025/26, which has resulted in some records showing as incorrectly having met the standard. ",
              "NHS Tayside has informed PHS that these data issues will be rectified once they have completed the ", 
              "migration to a new IT system. Due to the IT migration NHS Tayside have been unable to submit data from ",
              "all areas for 2025/26 Q3 and Q4 and 2026/27 Q1.*"), 
    TRUE ~ paste0(""))
  ...
  # Data quality notes - Orkney
   ork_date1 <- dmy(01042022)
   ork_date2 <- dmy(01042023)
  
  
  ork_text <-  
    case_when(sel_fy == extract_fin_year(ork_date1) | 
              sel_fy == extract_fin_year(ork_date2) ~
    paste0(" ", 
             "**Please note:** ",
  ...
  # Data quality notes - shetland
   shet_date1 <- dmy(01042022)
   shet_date2 <- dmy(01042023)
  
    shet_text <- 
    case_when(sel_fy == extract_fin_year(shet_date1) |
              sel_fy == extract_fin_year(shet_date2) ~
              paste0(" ", 
              "**Please note:** ",
    ...
    # Data quality notes - Grampian
    gramp_date1 <- dmy(01042023)
    gramp_date2 <- dmy(01042024)
    gramp_date3 <- dmy(01042025)
  
    gramp_text <- case_when(
      sel_fy == extract_fin_year(gramp_date1) |
        sel_fy == extract_fin_year(gramp_date2) |
        sel_fy == extract_fin_year(gramp_date3) ~
        paste0(" ", 
               "**Please note:** ",
               "*In 2025/26 Q4 submission the numbers reported for  2023/24, 2024/25 and 2025/26 by ", 
               "NHS Grampian are lower than those reported than in previous quarters. ", 
               "PHS are working with the Board to investigate and resolve this.*"), 
      TRUE ~ paste0(""))
...
flexdashboard::valueBox(paste0(round_half_up(exp_rate, 1), "%"), 
...
flexdashboard::valueBox(paste0(round_half_up(pds_rate, 1), "%"), 
...
### **Integration Authority Area - Number of Individuals relating to LDP Standard**: Financial Year `r paste0(sel_fy, table_title)`
