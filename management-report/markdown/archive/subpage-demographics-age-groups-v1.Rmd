...
ldp_all_age_2$age_grp_2<- factor(ldp_all_age_2$age_grp_2, levels = c("85+", "75 to 84" ,"74 and Under"))
...
pds_all_age_2_rates$age_grp_2<- factor(pds_all_age_2_rates$age_grp_2, levels = c("85+", "75 to 84" ,"74 and Under"))
...
ijb_pop_age_table_data <- bind_rows(
 pds_all_age_2_rates %>% filter(!grepl("NHS", geog)) %>% 
  ...
  select(geog,fy,age_grp_2,pop_rate_10000) %>% 
     mutate(pop_rate_10000 = as.character(pop_rate_10000)) %>%
  pivot_wider(names_from =age_grp_2, values_from = pop_rate_10000) %>% 
    mutate(age_2_group = "75 to 84", .before = fy),
 
 pds_all_age_2_rates %>% filter(!grepl("NHS", geog)) %>% 
  select(geog,fy,age_grp_2,pop_rate_10000) %>% 
     mutate(pop_rate_10000 = as.character(pop_rate_10000)) %>%
  pivot_wider(names_from =age_grp_2, values_from = pop_rate_10000) %>% 
    mutate(age_2_group = "74 and Under", .before = fy)
) %>% 
...
ijb_age_ldp_data <- ldp_all_age_2 %>% filter(!grepl("NHS", geog)) %>% 
  ...
  select(fy,geog,`85+`, `75 to 84`, `74 and Under`) %>% 
  mutate(across(everything(), ~replace_na(.x, "-"))) %>% 
...
ijb_age_totals_data <- pds_all_age_2 %>% filter(!grepl("NHS", geog)) %>% 
  ...
  relocate(`75 to 84`, .before = `74 and Under`) %>%  relocate(`85+`, .before = `75 to 84`) %>% 
