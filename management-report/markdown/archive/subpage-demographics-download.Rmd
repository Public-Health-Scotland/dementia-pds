...
ijb_age_ldp_data <- ldp_all_age_2 %>% filter(!grepl("NHS", geog)) %>% 
  ...
  select(fy,geog,`65+`,`64 and Under`) %>% 
  mutate(across(everything(), ~replace_na(.x, "-"))) %>% 
...
ijb_age_totals_data <- pds_all_age_2 %>% filter(!grepl("NHS", geog)) %>% 
  ...
  relocate(`65+`, .before = `64 and Under`) %>% 
...
DT::datatable(ijb_simd_table_pop, rownames = FALSE, selection = 'none', extensions = 'Buttons',
              options = list(buttons = list(list(
                                    extend = "collection", 
                                    buttons = list(list(
                                              extend = 'csv',
                                              filename = "pds_referrals_per_10,000_population_simd_iaa")),
                                    text = "Download table"
                                            )),
                             pageLength = 33,
                             scrollX = TRUE,
...
DT::datatable(hb_simd_table_pop, rownames = FALSE, selection = "none", #extensions = "FixedColumns",
                  options = list(buttons =
                        list(list(
                          extend = 'csv',
                          title = paste0("pds_referrals_per_10,000_population_simd_hb"),
