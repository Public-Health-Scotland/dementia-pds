...
```{r dropdown-pop-hb}
pds_hb_pop_data <- SharedData$new(pds_hb_rates %>% mutate(key = paste0(fy,age_grp_2), .before = geog),
                                  key = ~key, group = "grouphb")

data_filter_age_pop_hb <- filter_select(id = "trend_filter_age_grp_2_hb",
                                    label = "Select Age Group:",
                                    group = ~age_grp_2,
...
data_filter_year_pop_hb <- filter_select(id = "trend_filter_year_hb",
...
pds_scot_pop_data <- SharedData$new(pds_scot_rates %>% mutate(key = paste0(fy,age_grp_2), .before = geog),
 ...
 hb_rates_plot <- plot_bar_pop_rate_2(pds_hb_pop_data, pds_scot_pop_data)
...
```{r dropdown-pop-ijb, eval=FALSE, include=FALSE}
...
```{r plot-ijb-pop, eval=FALSE, include=FALSE}
