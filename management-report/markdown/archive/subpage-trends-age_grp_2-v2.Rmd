...
```{r dropdown-pop-hb}
pds_hb_rates_scot <- left_join(pds_hb_rates, pds_scot_rates %>% select(fy, age_grp_2, pop_rate_10000) %>% rename(scot_pop_rate_10000 = pop_rate_10000)) %>% mutate(key = paste0(geog, fy, age_grp_2, pop_rate_10000, scot_pop_rate_10000), .before = geog)

pds_hb_pop_data <- SharedData$new(pds_hb_rates_scot,
                                 key = ~key, group = "grouphb")

data_filter_age_trend_hb <- filter_select(id = "trend_filter_age_grp_2_hb",
                                    label = "Select Age Group:",
                                    group = ~age_grp_2,
...
# pds_scot_rates_key <- pds_scot_rates %>% mutate(key = paste0(geog,fy,age_grp_2), .before = geog)
# 
# pds_scot_pop_data <- SharedData$new(pds_scot_rates_key,
#                                     key = ~key, group = "grouphb")
 
 hb_rates_plot <- plot_bar_pop_rate_3(pds_hb_pop_data)#, pds_scot_pop_data)
...
pds_ijb_rates_scot <- left_join(pds_ijb_rates, pds_scot_rates %>% select(fy, age_grp_2, pop_rate_10000) %>% rename(scot_pop_rate_10000 = pop_rate_10000)) 

pds_ijb_pop_data <- SharedData$new(pds_ijb_rates_scot)
...
data_filter_age_trend_ijb <- filter_select(id = "trend_filter_age_grp_2_ijb",
                                    label = "Select Age Group:",
                                    group = ~age_grp_2,
                                    sharedData = pds_ijb_pop_data,
                                    multiple = FALSE
...
ijb_rates_plot <- plot_bar_pop_rate_2(pds_ijb_pop_data)
...
<!-- pds_all_rates %>% filter(geog == "Scotland" | grepl("NHS", geog), age_grp_2 == "All") %>%  -->
<!--             select(-age_grp_2) %>%  -->
<!--             pivot_wider(names_from = fy, values_from = pop_rate_10000) %>% -->
<!--             rename(`Health Board` = geog) %>% -->
<!--             kable(align = c("l", rep("r", length(unique(pds$fy))))) %>% -->
...
function filter_default() {
  ...
  document.getElementById("trend_filter_age_grp_2_hb").getElementsByClassName("selectized") 
  [0].selectize.setValue("All", false);

  document.getElementById("trend_filter_age_grp_2_hb").getElementsByClassName("selectized")
  [0].selectize.removeOption("");
  
  document.getElementById("trend_filter_age_grp_2_ijb").getElementsByClassName("selectized") 
  [0].selectize.setValue("All", false);
