...
pds_all_age_2_cal_year <- pds_all_age_2 %>%
  ...
  complete(nesting(fy,year, geog), age_grp_2, fill = list(total_referrals = 0)) 

pds_all_age_2_rates <- left_join(pds_all_age_2_cal_year, pop_data_age_2) %>% select(-year) %>%
  mutate(pop_rate_10000 = round((total_referrals/pop_est)*10000, 1)) %>% select(fy, geog, age_grp_2, total_referrals, pop_est, pop_rate_10000)
...
ijb_pop_age_table_data <- pds_all_age_2_rates %>%
  ...
  select(geog,fy,age_grp_2, total_referrals, pop_est, pop_rate_10000) %>%
   mutate(age_2_group = age_grp_2, .before = age_grp_2) %>% 
  mutate(key = paste0(fy,age_2_group), .before = geog) %>% 
     mutate(pop_rate_10000 = as.character(pop_rate_10000)) %>%
   mutate(total_referrals = as.character(total_referrals)) %>%
   mutate(pop_est = as.character(pop_est)) %>% 
  rename("Integration Authority Area" = geog, 
         "Number of referrals" = total_referrals,
         "Population Estimate" = pop_est,
         "Rate per 10,000" = pop_rate_10000,
         "Age Group" = age_grp_2)

#%>%
 # pivot_wider(names_from = geog, values_from = c(pop_rate_10000, total_referrals, pop_est)) %>%
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-30px"),
plot_geog_bar_v2(pds_pop_simd_data_ijb, y_value = pop_rate_10000, measure = simd, measure_text = "SIMD Quintile: ", measure_2 = simd_group, x = "Integration Authority Area", y = "Rate per 10,000 Population", y_value_text = "Rate per 10,000 Population: ", colours = simd_colours))
)
...
ijb_pop_simd_table_data <- bind_rows(
  ...
  pds_all_simd_rates %>%
  filter(!grepl("NHS", geog)) %>%
  select(geog,fy,simd, total_referrals, pop, pop_rate_10000) %>%
   mutate(simd_group = simd, .before = simd),
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-30px"),
plot_geog_bar_v2(pds_pop_simd_data_hb, y_value = pop_rate_10000, measure = simd, measure_text = "SIMD Quintile: ", measure_2 = simd_group, x = "Health Board", y = "Rate per 10,000 Population", y_value_text = "Rate per 10,000 Population: ", colours = simd_colours))
)
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-30px"),
plot_geog_bar_v2(ldp_simd_data_hb, y_value = perc_met, measure = simd, measure_text = "SIMD Quintile: ", x = "Health Board", measure_2 = simd_group, ylabel = function(x) paste0(x,"%"), ylimits = c(0, 101), symbol = "%", colours = simd_colours))
)
