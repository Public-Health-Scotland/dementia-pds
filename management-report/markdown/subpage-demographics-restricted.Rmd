...
ldp_all_age_2 <- pds_all %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
...
pds_all_age_2 <- pds_all %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
  filter(age_grp_2 != "Unknown") #remove 6 unknowns from 2016/17 (add note to charts and tables?)
...
pds_all_age <- pds_all %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
 filter(age_grp != "Unknown") #remove 6 unknowns from 2016/17 (add note to charts and tables?)
...
ldp_all_sex <- pds_all %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
...
pds_all_sex <- pds_all %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
  group_by(geog, fy, sex) %>% filter(sex != "98 Not Specified", sex != "99 Not Known", sex != "Unknown") %>% 
  summarise(total_referrals = sum(referrals), .groups = "drop") %>% 
...
#prepare simd data
ldp_all_simd <- pds_all %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
...
pds_all_simd <- pds_all %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
  group_by(geog, fy, simd) %>% filter(simd != "Unknown") %>% 
  summarise(total_referrals = sum(referrals), .groups = "drop") %>% 
...
hb_totals_table <- bind_rows(pds_scot_totals, pds_hb_totals) %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
  mutate(total_referrals = format(total_referrals, big.mark = ",")) %>% pivot_wider(names_from = geog, values_from = total_referrals)

ijb_totals_table <- bind_rows(pds_scot_totals, pds_ijb_totals)  %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
  mutate(total_referrals = format(total_referrals, big.mark = ",")) %>% pivot_wider(names_from = geog, values_from = total_referrals)
...
```{r plot-prop-age-ijb}
bscols(widths = 12,
  div(style = css(width="98%", height="410px", marginBottom="-10px"),
...
```{r plot-prop-age-hb}
bscols(widths = 12,
  div(style = css(width="98%", height="400px", marginBottom="0px"),
...
bscols(widths = 12,
       div(style = css(width="98%", height="260px", margin="15px"),
...
```{r plot-ijb-sex-pop}
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-10px"),
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-10px"),
plot_geog_bar_v2(pds_pop_simd_data_ijb, y_value = pop_rate_10000, measure = simd, measure_text = "SIMD Quintile: ", x = "Integration Authority Area", y = "Rate per 10,000 Population", y_value_text = "Rate per 10,000 Population: "))
)
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px"> Number of people referred to PDS per 10,000 population (18+) by SIMD Quintile and Health Board</h2>
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-10px"),
plot_geog_bar(pds_pop_simd_data_hb, y_value = pop_rate_10000, measure = simd, measure_text = "SIMD Quintile: ", x = "Health Board", y = "Rate per 10,000 Population", y_value_text = "Rate per 10,000 Population: "))
)
...
ldp_simd_data_ijb <- SharedData$new(ldp_ijb_simd, key = ~fy, group = "ijb_simd_ldp")
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-10px"),
plot_geog_bar(ldp_simd_data_ijb, y_value = perc_met, measure = simd, measure_text = "SIMD Quintile: ", x = "Integration Authority Area", ylabel = function(x) paste0(x,"%"), ylimits = c(0, 101), symbol = "%"))
)
...
ijb_ldp_simd_table_data <- ldp_all_simd %>%
  filter(!grepl("NHS", geog)) %>% 
...
ijb_simd_table_ldp <- SharedData$new(ijb_ldp_simd_table_data, key = ~fy, group = "ijb_simd_ldp")
...
<br>
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px">Number of referrals to PDS by SIMD Quintile and Integration Authority Area</h2>
```{r table-ijb-simd-totals}

ijb_simd_totals_data <- pds_all_simd %>%
  filter(!grepl("NHS", geog)) %>% 
...
ijb_simd_totals <- SharedData$new(ijb_simd_totals_data, key = ~fy, group = "ijb_simd_ldp")


bscols(widths = 12,
       div(style = css(width="97%", height="300px", margin="20px"),
DT::datatable(ijb_simd_totals, rownames = FALSE, extensions = "FixedColumns",
              options = list(pageLength = 5,
                             scrollX = TRUE,
                             fixedColumns = list(leftColumns = 1),
                             scrollY = FALSE,
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-10px"),
plot_geog_bar_v2(ldp_simd_data_hb, y_value = perc_met, measure = simd, measure_text = "SIMD Quintile: ", x = "Health Board", ylabel = function(x) paste0(x,"%"), ylimits = c(0, 101), symbol = "%"))
)
...
hb_simd_ldp_data <- bind_rows(
  ...
  mutate(key = paste0(fy,simd_group), .before = geog) %>% 
   ...
   pivot_wider(names_from = geog, values_from = perc_met) %>%
  mutate(across(everything(), ~replace_na(.x, "-"))) %>%
  mutate(type = "% Standard Met", .before = Scotland) %>% 
             rename(`SIMD Quintile` = simd)
...
hb_ldp_simd_table_data <- bind_rows(hb_simd_totals_data, hb_simd_ldp_data) %>% 
  ...
  mutate(`SIMD Quintile` = if_else(type == "% Standard Met", " ", `SIMD Quintile`)) %>% 
 rename(" " = "type")
...
bscols(widths = 12,
       div(style = css(width="97%", height="235px", margin="20px", marginTop="-25px"),
