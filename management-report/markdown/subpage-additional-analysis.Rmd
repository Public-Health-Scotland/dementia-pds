...
ldp_all_model <- model_data %>% filter(age_grp_2 == "All", sex == "All", simd == "All") %>%
  select(-health_board, -age_grp_2, -total_referrals) %>% 
...
bscols(widths = 12,
div(style=css(marginBottom="-23px",height="95px", width="90%"), plot_prop_legend(pds_all_subtype %>% filter(geog == "Scotland"), measure = subtype, measure_text = "Subtype of Dementia: "))
)
```
...
bscols(widths = 12,
div(style=css(marginBottom="-20px",height="70px"), plot_prop_legend(pds_all_stage %>% filter(geog == "Scotland"), measure = stage, measure_text = "Stage of Illness: ")))
...
bscols(widths = 12,
div(style=css(marginBottom="-23px",height="95px", width="90%"), plot_prop_legend(pds_all_model %>% filter(geog == "Scotland"), measure = model, measure_text = "Model of Care: ")))
...
data_filter_prop_model_geog <- filter_select(id = "filter_prop_model_age_geog",
                                    label = "Select to view Integration Authorities or Health Boards",
                                    group = ~geog_type,
...
bscols(widths = 12, div(style=css(marginBottom="-23px",height="95px", width="90%"), plot_prop_legend(pds_all_model %>% filter(geog == "Scotland"), measure = model, measure_text = "Model of Care: ")))
...
bscols(widths = 12,
  div(style = css(width="98%", height="400px"),
      plot_prop_2(pds_model_age_data, measure = model, measure_text = "Model of Care: ", measure_2 = age_grp_2)
)
)
```
...
xlabel_data <- tibble(
  geog_type = rep(c("Integration Authority Areas", "Health Boards"),3),
  geog_type_2 = rep(c("Integration Authority Area", "Health Board"),3),
  age_grp_2 = rep(c("All", "65+", "64 and Under"),2),
  x = c(1,2,3,4,5,6),
  y = c(0,0,0,0,0,0))

xlabel_data_shared <- SharedData$new(xlabel_data %>% mutate(key = paste0(age_grp_2, geog_type)),
                                   key = ~key, group = "model_prop_age")

bscols(widths = c(6,3,NA), NULL, div(style=css(marginBottom="20px", marginTop="-35px",marginLeft="-107px",height="30px",width="250px"),plot_prop_2_label(xlabel_data_shared)),NULL)
...
bscols(widths = 12,
  div(style = css(width="98%", height="400px"),
      plot_prop_2(pds_model_sex_data, measure = model, measure_text = "Model of Care: ", measure_2 = sex)
)
)
```
