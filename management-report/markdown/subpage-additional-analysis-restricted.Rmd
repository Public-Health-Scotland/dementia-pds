...
pds_all_subtype <- subtype_data %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
  filter(age_grp_2 == "All") %>% 
...
ldp_all_subtype <- subtype_data %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
  filter(age_grp_2 == "All") %>%
...
pds_all_stage <- stage_data %>% filter(age_grp_2 == "All") %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
...
ldp_all_stage <- stage_data %>% filter(age_grp_2 == "All") %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
  select(-health_board, -age_grp_2, -total_referrals) %>% 
...
pds_all_model <- model_data %>% filter(age_grp_2 == "All", sex == "All", simd == "All") %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
...
ldp_all_model <- model_data %>% filter(age_grp_2 == "All", sex == "All", simd == "All") %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% 
  select(-health_board, -age_grp_2, -total_referrals) %>% 
...
uptake_data <- read_rds(get_mi_data_path("uptake_data", ext = "rds", test_output = test_output)) %>% 
  filter(ijb %in% c(restricted_ijb,"All"), health_board %in% restricted_hb)
...
bind_rows(pds_scot_totals_2, pds_ijb_totals_2) %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% group_by(geog, fy = "All", age_grp_2) %>% 
...
bind_rows(pds_scot_totals_2, pds_hb_totals_2) %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% group_by(geog, fy = "All", age_grp_2) %>% 
  summarise(`All referrals` = sum(total_referrals), .groups = "drop") %>% 
...
bscols(widths = 12,
       div(style = css(width="98%", height="600px", margin="15px"),
...
bscols(widths = 12,
       div(style = css(width="98%", height="470px", margin="15px"),
...
bscols(widths = 12,
       div(style = css(width="98%", height="490px", margin="15px"),
...
```{r dropdown-prop-model-age, eval=FALSE, include=FALSE}
...
```{r plot-prop-model-legend-age, eval=FALSE, include=FALSE}
...
```{r plot-prop-model-age, eval=FALSE, include=FALSE}
bscols(widths = 12,
  div(style = css(width="98%", height="400px"),
...
```{r plot-prop-model-age-label, eval=FALSE, include=FALSE}
...
```{r table-prop-model-age, eval=FALSE, include=FALSE}
...
bscols(widths = 12,
       div(style = css(width="98%", height="530px", margin="15px"),
...
```{r dropdown-prop-model-sex, eval=FALSE, include=FALSE}
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px">Proportion of Referrals by Model of Care for All Financial Years (2016/17 - `r current_fy`)</h2>
```{r plot-prop-model-legend-sex, eval=FALSE, include=FALSE}
...
```{r plot-prop-model-sex, eval=FALSE, include=FALSE}
bscols(widths = 12,
  div(style = css(width="98%", height="400px"),
...
```{r plot-prop-model-sex-label, eval=FALSE, include=FALSE}
...
```{r table-prop-model-sex, eval=FALSE, include=FALSE}
...
```{r dropdown-prop-model-simd, eval=FALSE, include=FALSE}
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px">Proportion of Referrals by Model of Care for All Financial Years (2016/17 - `r current_fy`)</h2>
```{r plot-prop-model-legend-simd, eval=FALSE, include=FALSE}
...
```{r plot-prop-model-simd, eval=FALSE, include=FALSE}
bscols(widths = 12,
  div(style = css(width="98%", height="400px"),
...
```{r plot-prop-model-simd-label, eval=FALSE, include=FALSE}
...
```{r table-prop-model-simd, eval=FALSE, include=FALSE}
...
```{r dropdown-uptake-ijb}
 ...
 table_ijb_uptake_data <- uptake_data %>% 
  ...
  rename("Number of People Referred to PDS" = "Total",
             "Integration Authority Area" = "ijb",
             "Percentage Accepted (includes those initially declined)" = "perc_accepted")
