...
#   pds_all %>%
#   filter(fy %in% full_years, sex != "98 Not Specified", sex != "99 Not Known", sex != "Unknown", age_grp_2 != "Unknown") %>%
...
data_filter_age_prop <- filter_select(id = "filter_age_prop_ijb",
...
bscols(widths = 12,
div(style=css(marginBottom="-20px",height="70px"), plot_prop_legend(pds_all_age %>% filter(geog == "Scotland"), measure = age_grp)))
...
bscols(widths = 12,
  div(style = css(width="98%", height="400px"),
      plot_prop(pds_age_data_ijb, measure = age_grp, measure_text = "Gender: ", xlabel = "Integration Authority Area")
)
)
...
```{r table-prop-age-ijb}

pds_prop_table_data_ijb <- bind_rows(
  
  ijb_totals_table %>% mutate(age_grp_2 = "All referrals",
                              across(everything(), as.character)) %>% rename(`Gender` = age_grp_2),
  
  pds_all_age %>% filter(!grepl("NHS", geog)) %>% group_by(geog, fy) %>% 
...
bscols(widths = 12,
       div(style = css(width="98%", height="480px", margin="15px"),
...
data_filter_age_prop <- filter_select(id = "filter_age_prop_hb",
                                    label = "Select Financial Year of Diagnosis:",
                                    group = ~fy,
...
pds_prop_table_data_hb <- bind_rows(
  ...
  hb_totals_table %>% mutate(age_grp_2 = "All referrals",
                              across(everything(), as.character)) %>% rename(`Age Group` = age_grp_2),
  
  pds_all_age %>% filter(geog == "Scotland" | grepl("NHS", geog)) %>%
  group_by(geog, fy) %>% 
...
bscols(widths = 12,
       div(style = css(width="98%", height="480px", margin="15px", marginTop="0px"),
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px"> Number of people referred to PDS per 10,000 population (18+) by Age Group and Integration Authority Area</h2>
```{r plot-ijb-age-pop}
...
bscols(widths = 12,
       div(style = css(width="97%", height="160px", margin="20px"),
DT::datatable(ijb_age_table_pop, rownames = FALSE, extensions = "FixedColumns",
              options = list(pageLength = 5,
                             ...
                             columnDefs = list(list(visible = FALSE, targets = 0:2),
                                               list(className = 'dt-right', targets = 4:35)
                                               ))) %>%
           DT::formatStyle(4, fontWeight = "bold", padding = "4px"
                          )
        )
     )
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px"> Number of people referred to PDS per 10,000 population (18+) by Age Group and Health Board</h2>
```{r plot-hb-age-pop}
...
hb_pop_age_table_data <-   pds_all_age_2_rates %>%
...
DT::datatable(hb_age_table_pop, rownames = FALSE, extensions = "FixedColumns",
              options = list(pageLength = 5,
                             ...
                             ordering = FALSE,
                             columnDefs = list(list(visible = FALSE, targets = 0:2),
                                               list(className = 'dt-right', targets = 4:18)
                                               ))) %>%
           DT::formatStyle(4, fontWeight = "bold", padding = "4px"
                          )
        )
...
plot_rate_line(pds_pop_age_2_data, measure = age_grp_2, measure_text = "Age Group: ", y = "Rate per 10,000 Population", scales = "free_y"))
)
...
bscols(widths = 12,
div(style=css(marginBottom="0px",height="23px"), plot_bar_age_pop_legend(age_pop_legend_data)),NULL
)
```
...
bscols(widths = 12,
        div(style = css(width="98%", height="140px", margin="15px", marginTop="-10px"),
...
ijb_age_ldp_data <- ldp_all_age_2 %>% filter(!grepl("NHS", geog)) %>% 
  select(geog,fy,age_grp_2,perc_met) %>% 
   ...
   pivot_wider(names_from = geog, values_from = perc_met) %>%
  mutate(across(everything(), ~replace_na(.x, "-"))) %>%
  mutate(type = "% Met Standard/Exempt", .before = Scotland) %>% 
             rename(`Age Group` = age_grp_2)


ijb_age_totals_data <- pds_all_age_2 %>% filter(!grepl("NHS", geog)) %>% 
  pivot_wider(names_from = geog, values_from = total_referrals) %>%
  mutate(across(everything(), ~replace_na(.x, 0))) %>% 
  mutate(across(where(is.numeric), ~format(., big.mark = ","))) %>%
  mutate(type = "Number of Referrals", .before = Scotland) %>%
             rename(`Age Group` = age_grp_2)


ijb_ldp_age_table_data <- bind_rows(ijb_age_totals_data, ijb_age_ldp_data) %>% 
  arrange(fy, `Age Group`) %>% 
  mutate(`Age Group` = if_else(type == "% Met Standard/Exempt", " ", `Age Group`)) %>% 
 rename(" " = "type")
...
bscols(widths = 12,
       div(style = css(width="97%", height="310px", margin="20px"),
DT::datatable(ijb_age_table_ldp, rownames = FALSE, extensions = "FixedColumns",
               options = list(pageLength = 10,
                             ...
                             ordering = FALSE,
                             columnDefs = list(list(visible = FALSE, targets = 0),
                                               list(className = 'dt-right', targets = 3:34)
                             ))) %>%
...
DT::datatable(hb_age_table_ldp, rownames = FALSE, extensions = "FixedColumns",
              options = list(pageLength = 10,
                             ...
                             ordering = FALSE,
                             columnDefs = list(list(visible = FALSE, targets = 0),
                                               list(className = 'dt-right', targets = 3:17)
                                               ))) %>%
           DT::formatStyle(2:3, fontWeight = "bold", padding = "4px"
                          )
        )
...
ldp_age_2_table_data <- bind_rows(trend_age_2_totals, ldp_age_2_trend) %>% arrange(geog, age_grp_2) %>% 
            mutate(across(everything(), ~replace_na(.x, "-"))) %>% 
...
Row {.tabset data-height=870}
----------------------------------------------------------------------

### **Integration Authority Areas**
```{r dropdown-prop-sex-ijb}
pds_sex_data_ijb <- SharedData$new(pds_all_sex %>% filter(!grepl("NHS", geog)), key = ~fy, group = "sex_prop_ijb")
...
bscols(widths = 12,
div(style=css(marginBottom="-20px",height="65px"), plot_prop_legend(pds_all_sex %>% filter(geog == "Scotland"), measure = sex)))
...
bscols(widths = 12,
  div(style = css(width="98%", height="400px"),
      plot_prop(pds_sex_data_ijb, measure = sex, measure_text = "Gender: ", xlabel = "Integration Authority Area")
)
)
...
```{r ttable-prop-sex-ijb}

pds_prop_sex_table_data_ijb <- bind_rows(
  
  ijb_totals_table %>% mutate(age_grp_2 = "All referrals",
                              across(everything(), as.character)) %>% rename(`Gender` = age_grp_2),
  
  pds_all_sex %>% filter(!grepl("NHS", geog)) %>% group_by(geog, fy) %>% 
...
pds_prop_sex_table_ijb <- SharedData$new(pds_prop_sex_table_data_ijb, key = ~fy, group = "sex_prop_ijb")

bscols(widths = 12,
       div(style = css(width="98%", height="230px", margin="15px"),
...
pds_sex_data_hb <- SharedData$new(pds_all_sex %>% filter(geog == "Scotland" | grepl("NHS", geog)),
                                   key = ~fy, group = "sex_prop_hb")

data_filter_sex_prop <- filter_select(id = "filter_sex_prop_hb",
                                    label = "Select Financial Year of Diagnosis:",
                                    group = ~fy,
...
bscols(widths = 12,
  div(style = css(width="98%", height="400px"),
      plot_prop(pds_sex_data_hb, measure = sex, measure_text = "Gender: ", xlabel = "Health Board")
)
)
...
pds_prop_sex_table_data_hb <- bind_rows(
  
  hb_totals_table %>% mutate(age_grp_2 = "All referrals",
                              across(everything(), as.character)) %>% rename(`Gender` = age_grp_2),
  
  pds_all_sex %>% filter(geog == "Scotland" | grepl("NHS", geog)) %>%
  group_by(geog, fy) %>% 
    mutate(perc_referrals = paste0(round((total_referrals/sum(total_referrals))*100,0),"%")) %>%
    ungroup() %>% 
  select(-total_referrals) %>% 
...
pds_prop_sex_table_hb <- SharedData$new(pds_prop_sex_table_data_hb, key = ~fy, group = "sex_prop_hb")

bscols(widths = 12,
       div(style = css(width="98%", height="260px", margin="15px", marginTop="0px"),
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px"> Number of people referred to PDS per 10,000 population (18+) by Gender and Integration Authority Area</h2>
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px"> Number of people referred to PDS per 10,000 population (18+) by Gender and Health Board</h2>
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 15px"> Number of people referred to PDS per 10,000 population (18+) by Gender and Financial Year</h2>
...
ijb_ldp_sex_table_data <- bind_rows(ijb_sex_totals_data, ijb_sex_ldp_data) %>% 
  arrange(fy, `Gender`) %>% 
  mutate(`Gender` = if_else(type == "% Met Standard/Exempt", " ", `Gender`)) %>% 
 rename(" " = "type")
...
bscols(widths = 12,
div(style=css(marginBottom="-20px",height="65px"), plot_prop_legend(pds_all_simd %>% filter(geog == "Scotland"), measure = simd)))
...
```{r table-prop-simd-ijb}

pds_prop_simd_table_data_ijb <- bind_rows(
  
  ijb_totals_table %>% mutate(age_grp_2 = "All referrals",
                              across(everything(), as.character)) %>% rename(`SIMD Quintile` = age_grp_2),
  
  pds_all_simd %>% filter(!grepl("NHS", geog)) %>% group_by(geog, fy) %>% 
  mutate(perc_referrals = paste0(round((total_referrals/sum(total_referrals))*100,0),"%")) %>% ungroup() %>%   select(-total_referrals) %>% 
  pivot_wider(names_from = geog, values_from = perc_referrals) %>%
...
bscols(widths = 12,
       div(style = css(width="98%", height="380px", margin="15px"),
...
```{r table-prop-simd-hb}

pds_prop_simd_table_data_hb <- bind_rows(
  
  hb_totals_table %>% mutate(age_grp_2 = "All referrals",
                              across(everything(), as.character)) %>% rename(`SIMD Quintile` = age_grp_2),
  
  pds_all_simd %>% filter(geog == "Scotland" | grepl("NHS", geog)) %>%
  group_by(geog, fy) %>% 
    mutate(perc_referrals = paste0(round((total_referrals/sum(total_referrals))*100,0),"%")) %>%
    ungroup() %>% 
  select(-total_referrals) %>% 
...
bscols(widths = 12,
       div(style = css(width="98%", height="390px", margin="15px", marginTop="0px"),
...
pds_ijb_simd_rates <- bind_rows(
  ...
  mutate(key = paste0(fy,simd_group), .before = geog),
...
bscols(widths = c(1,5,5,NA),
       NULL,
       data_filter_ijb_simd_pop,
       data_filter_ijb_simd_pop_quintile,
       NULL)


```

<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px"> Number of people referred to PDS per 10,000 population (18+) by SIMD Quintile and Integration Authority Area</h2>
```{r plot-ijb-simd-legend-pop}

bscols(widths = 12,
div(style=css(marginBottom="-20px",height="65px"), plot_geog_bar_legend(pds_all_simd %>% filter(geog == "Scotland"), measure = simd)))
...
# bscols(widths = 12,
#   div(style = css(width="98%", height="385px", marginBottom="-30px"),
# plot_geog_bar(pds_pop_simd_data_ijb, y_value = pop_rate_10000, measure = simd, measure_text = "SIMD Quintile: ", x = "Integration Authority Area", y = "Rate per 10,000 Population", y_value_text = "Rate per 10,000 Population: "))
# )

bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-30px"),
plot_geog_bar_v2(pds_pop_simd_data_ijb, y_value = pop_rate_10000, measure = simd, measure_text = "SIMD Quintile: ", measure_2 = simd_group, x = "Integration Authority Area", y = "Rate per 10,000 Population", y_value_text = "Rate per 10,000 Population: "))
)
...
bscols(widths = c(1,5,5,NA),
       ...
       data_filter_hb_simd_pop_quintile,
       NULL)


```

<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px"> Number of people referred to PDS per 10,000 population (18+) by SIMD Quintile and Heath Board</h2>
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-30px"),
plot_geog_bar_v2(pds_pop_simd_data_hb, y_value = pop_rate_10000, measure = simd, measure_text = "SIMD Quintile: ", measure_2 = simd_group, x = "Health Board", y = "Rate per 10,000 Population", y_value_text = "Rate per 10,000 Population: "))
)
...
bscols(widths = 12,
       div(style = css(width="97%", height="360px", margin="20px", marginTop="10px"),
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 15px"> Number of people referred to PDS per 10,000 population (18+) by SIMD Quintile and Financial Year</h2> 
...
bscols(widths = 12,
  div(style = css(width="98%", height="370px"),
plot_rate_line(pds_pop_simd_data, measure = simd, measure_text = "SIMD Quintile: ", y = "Rate per 10,000 Population"))
)
...
bscols(widths = c(1,5,5,NA),
       ...
       data_filter_ijb_simd_ldp_quintile,
       NULL)
...
```{r plot-ijb-simd-legend-ldp}
 ...
 bscols(widths = 12,
div(style=css(marginBottom="-20px",height="65px"), plot_geog_bar_legend(ldp_all_simd %>% filter(geog == "Scotland"), measure = simd)))
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-30px"),
plot_geog_bar_v2(ldp_simd_data_ijb, y_value = perc_met, measure = simd, measure_text = "SIMD Quintile: ", x = "Integration Authority Area", measure_2 = simd_group, ylabel = function(x) paste0(x,"%"), ylimits = c(0, 101), symbol = "%"))
)
...
ijb_ldp_simd_table_data <- bind_rows(ijb_simd_totals_data, ijb_simd_ldp_data) %>% 
  arrange(`SIMD Quintile`, key, simd_group) %>% 
...
DT::datatable(ijb_simd_table_ldp, rownames = FALSE, extensions = "FixedColumns",
              options = list(pageLength = 10,
                             scrollX = TRUE,
                             fixedColumns = list(leftColumns = 2),
                             scrollY = FALSE,
                             ...
                             ordering = FALSE,
                             columnDefs = list(list(visible = FALSE, targets = 0:2),
                                               list(className = 'dt-right', targets = 5:36)
                                               ))) %>%
           DT::formatStyle(4:5, fontWeight = "bold"#, padding = "4px"
                          )
        )
     )
...
bscols(widths = c(1,5,5,NA),
       ...
       data_filter_hb_simd_ldp_quintile,
       NULL)

# ldp_hb_simd <- ldp_all_simd %>% filter(geog == "Scotland" | grepl("NHS", geog)) 
# 
# ldp_simd_data_hb <- SharedData$new(ldp_hb_simd, key = ~fy, group = "hb_simd_ldp")
# 
# data_filter_hb_simd_ldp <- filter_select(id = "filter_hb_simd_ldp",
#                                     label = "Select Financial Year:",
#                                     group = ~fy,
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-30px"),
plot_geog_bar_v2(ldp_simd_data_hb, y_value = perc_met, measure = simd, measure_text = "SIMD Quintile: ", x = "Health Board", measure_2 = simd_group, ylabel = function(x) paste0(x,"%"), ylimits = c(0, 101), symbol = "%"))
)
...
bscols(widths = 12,
       div(style = css(width="97%", height="400px", margin="20px"),
DT::datatable(hb_simd_table_ldp, rownames = FALSE, extensions = "FixedColumns",
              options = list(pageLength = 10,
                             scrollX = TRUE,
                             fixedColumns = list(leftColumns = 2),
                             scrollY = FALSE,
                             ...
                             ordering = FALSE,
                             columnDefs = list(list(visible = FALSE, targets = 0:2),
                                               list(className = 'dt-right', targets = 5:19)
                                               ))) %>%
           DT::formatStyle(4:5, fontWeight = "bold"#, padding = "4px"
                          )
        )
...
# 
# hb_ldp_simd_table_data <- ldp_all_simd %>% filter(geog == "Scotland" | grepl("NHS", geog)) %>% 
#   select(geog,fy,simd,perc_met) %>%
#   mutate(perc_met = if_else(is.na(perc_met), as.character(perc_met), paste0(perc_met, "%"))) %>% 
#   pivot_wider(names_from = geog, values_from = perc_met) %>%
#   mutate(across(everything(), ~replace_na(.x, "-"))) %>% 
...
# 
# 
# hb_simd_table_ldp <- SharedData$new(hb_ldp_simd_table_data, key = ~fy, group = "hb_simd_ldp")
# 
# 
# bscols(widths = 12,
...
plot_ldp_bar_legend(ldp_all_simd, measure = simd, measure_text = ""))
)
...
plot_ldp_bar(ldp_all_simd_data_2, measure = simd, measure_text = "SIMD Quintile: "))
)
...
DT::datatable(ldp_simd_table_2, rownames = FALSE,
              options = list(pageLength = 10,
                             scrollX = TRUE,
                             scrollY = FALSE,
                             dom = "t",
...
function filter_default() {
  ...
  document.getElementById("filter_age_prop_hb").getElementsByClassName("selectized") 
  [0].selectize.setValue("2023/24", false);

  document.getElementById("filter_age_prop_hb").getElementsByClassName("selectized")
  [0].selectize.removeOption("");
  
  document.getElementById("filter_age_prop_ijb").getElementsByClassName("selectized") 
  [0].selectize.setValue("2023/24", false);
  ...
  document.getElementById("filter_hb_simd_pop_quintile").getElementsByClassName("selectized") 
  ...
  document.getElementById("filter_ijb_simd_ldp_quintile").getElementsByClassName("selectized") 
  [0].selectize.setValue("All", false);
