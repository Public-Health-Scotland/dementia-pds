...
pds_all_simd_rates <- left_join(pds_all_simd_cal_year, pop_data_simd) %>% select(-year) %>%
  mutate(pop_rate_10000 = round((total_referrals/pop)*10000, 2)) %>% select(geog, fy, simd, pop_rate_10000)
...
pds_age_data_ijb <- SharedData$new(pds_ijb_age, key = ~fy, group = "age_prop")
...
<h2 style="margin-left: 60px; margin-top: 0px; margin-bottom: 0px"> Proportion of Referrals by Age Group </h2>

```{r plot-prop-age-legend-ijb}

bscols(widths = 12,
div(id="ageproplegend", plot_prop_legend(pds_ijb_age), style="margin-bottom:-353px"))
...
bscols(widths = 12,
  div(style = css(width="98%", height="530px"),
      plot_prop(pds_age_data_ijb, xlabel = "Integration Authority Area")
)
)
...
pds_prop_table_data <- pds_all_age %>% filter(!grepl("NHS", geog)) %>% group_by(geog, fy) %>% 
...
pds_prop_table <- SharedData$new(pds_prop_table_data, key = ~fy, group = "age_prop")

bscols(widths = 12,
       div(style = css(width="98%", height="430px", margin="15px"),
          DT::datatable(pds_prop_table, rownames = FALSE, extensions = "FixedColumns",
...
pds_pop_simd_data <- SharedData$new(pds_all_simd_rates, key = ~geog, group = "trend_simd_pop")
...
<h2 style="margin-left: 50px; margin-top: 0px; margin-bottom: 0px"> Number of people referred to PDS per 10,000 population by SIMD Quintile and Financial Year
 </h2>
...
plot_trend_simd(pds_pop_simd_data, measure = pop_rate_10000, y = "Rate per 10,000 Population"))
)
...
```{r plot-hb-simd-legend-pop}
 ...
 bscols(widths = 12,
 div(id="popsimdlegendhb",plot_geog_simd_legend(pds_all_simd), style="margin-bottom:-355px"))
...
```{r plot-ijb-simd-legend-pop}
 ...
 bscols(widths = 12,
 div(id="popsimdlegendijb",plot_geog_simd_legend(pds_pop_simd_data_ijb), style="margin-bottom:-355px"))
