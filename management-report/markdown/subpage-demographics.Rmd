...
#prepare population rate data fo simd
...
hb_totals_table <- bind_rows(pds_scot_totals, pds_hb_totals) %>%pivot_wider(names_from = geog, values_from = total_referrals)

ijb_totals_table <- bind_rows(pds_scot_totals, pds_ijb_totals) %>% pivot_wider(names_from = geog, values_from = total_referrals)
   
hb_totals_table_2 <- bind_rows(pds_scot_totals_2, pds_hb_totals_2) %>%pivot_wider(names_from = geog, values_from = total_referrals)

ijb_totals_table_2 <- bind_rows(pds_scot_totals_2, pds_ijb_totals_2) %>% pivot_wider(names_from = geog, values_from = total_referrals)
...
bscols(widths = 12,
div(style=css(marginBottom="-20px",height="70px"), plot_prop_legend(pds_all_age %>% filter(geog == "Scotland"), measure = age_grp, measure_text = "Age Group: ")))
...
plot_trend_rate_legend_1(legend_data)),NULL
)
...
bscols(widths = 12,
div(style=css(marginBottom="-20px",height="65px"), plot_prop_legend(pds_all_sex %>% filter(geog == "Scotland"), measure = sex, measure_text = "Gender: ")))
...
bscols(widths = 12,
div(style=css(marginBottom="-20px",height="65px"), plot_prop_legend(pds_all_simd %>% filter(geog == "Scotland"), measure = simd, measure_text = "SIMD Quintile: ")))
...
ijb_ldp_simd_table_data <- ldp_all_simd %>%
  filter(!grepl("NHS", geog)) %>% 
  select(geog,fy,simd,perc_met) %>% mutate(perc_met = as.character(perc_met)) %>% 
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-30px"),
plot_geog_bar(ldp_simd_data_hb, y_value = perc_met, measure = simd, measure_text = "SIMD Quintile: ", x = "Health Board", ylabel = function(x) paste0(x,"%"), ylimits = c(0, 101), symbol = "%"))
)
