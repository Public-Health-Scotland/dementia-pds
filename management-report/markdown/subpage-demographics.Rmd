...
# AGE
################################################################################.
# Referral Rates by Age (Per 10,000 Population)
referral_rates_age <- standardised_rates_age %>% 
  select(fy, geog, name, age_grp_2, all_of(contains("rate"))) %>%
  pivot_longer(cols = contains("rate"), names_to = "rate_type", values_to = "rate") %>%
  mutate(rate = round(rate, 1))

# Outcomes by Age (Percentage Standard Met)
outcomes_age <- measures_age %>%
  select(fy, geog, name, age_grp_2, perc_met)

################################################################################.
# SEX
################################################################################.
# Referral Rates by Sex (Per 10,000 Population)
referral_rates_sex <- standardised_rates_sex %>% 
  select(fy, geog, name, sex, all_of(contains("rate"))) %>%
  pivot_longer(cols = contains("rate"), names_to = "rate_type", values_to = "rate") %>%
  mutate(rate = round(rate, 1))

# Outcomes by Sex (Percentage Standard Met)
outcomes_sex <- measures_sex %>%
  select(fy, geog, name, sex, perc_met)

################################################################################.
# SIMD
################################################################################.
# Referral Rates by SIMD (Per 10,000 Population)
referral_rates_simd <- standardised_rates_simd %>% 
  select(fy, geog, name, simd, all_of(contains("rate"))) %>%
  pivot_longer(cols = contains("rate"), names_to = "rate_type", values_to = "rate") %>%
  mutate(rate = round(rate, 1))

# Outcomes by SIMD (Percentage Standard Met)
outcomes_simd <- measures_simd %>%
  select(fy, geog, name, simd, perc_met)

plot_demog_bar <- function(data, x, y, demog){
  colours <- if (demog == "simd") {
    c("#0062AD", "#0078D4", "#3393DD", "#80BCEA", "#B3D7F2")
    } else if (demog == "sex") {
      c("#3F3685", "#9B4393")
    } else if (demog == "age_grp_2") {
      c("#6B5C85", "#0078D4", "#1E7F84")
    }

  text_label <- if (demog == "simd") {
  ...
  bscols(widths = 12, 
         div(style = css(width="98%", height="350px", marginBottom="-20px"),
             plot_ly(data, 
                     ...
                     y = data[[y]], 
                     meta = ~fy, 
                     hovertext = data[[demog]], 
                     type = 'bar', 
                     color = data[[demog]], 
                     colors = colours, 
                     hovertemplate = paste0("%{x} <br>", "%{meta} <br>", text_label, ": %{hovertext} <br>",
                                            "Rate per 10,000 Population: %{y}<extra></extra>")) %>%
               layout(clickmode = "none", 
                      showlegend = TRUE, 
                      legend = list(orientation = 'h', x = 0.5, y = 100, xanchor = "center"),
...
ldp_all_age <- pds_all %>%
  ...
  mutate(perc_met = round_half_up((complete + exempt)/(complete + exempt + fail)*100,1)) %>% 
...
pds_all_age_rates <- left_join(pds_all_age_cal_year, pop_data_age) %>% select(-year) %>%
  mutate(pop_rate_10000 = round_half_up((total_referrals/pop_est)*10000, 1)) %>% select(fy, geog, age_grp_2, pop_rate_10000)
...
pds_all_sex_rates <- left_join(pds_all_sex_cal_year, pop_data_sex) %>% select(-year) %>%
  mutate(pop_rate_10000 = round_half_up((total_referrals/pop_est)*10000, 1))
...
simd_pop_data <- read_rds(get_pop_lookup_path(simd = TRUE))
...
pds_all_simd_rates <- left_join(pds_all_simd_cal_year, pop_data_simd) %>% select(-year) %>%
  mutate(pop_rate_10000 = round_half_up((total_referrals/pop)*10000, 1))
...
legend_data_1 <- tibble(
  geog = unique((pds_all %>% filter(geog != "Unknown"))$geog),
...
legend_data_2 <- tibble(
  geog = unique((pds_all %>% filter(geog != "Unknown"))$geog),
  legend = unique((pds_all %>% filter(geog != "Unknown"))$geog),
  x = 1,
  y = 0)
...
# Dynamic Plot Data (Age-Stratified Referral Rates by IAA and Rate Type)
rates_df_age_ijb <- SharedData$new(
  referral_rates_age %>%
    filter(name %in% c("scot", "ijb")), 
  key = ~paste0(fy, rate_type), 
  group = "ratesageijb")

# Dropdown (Year)
rates_filter_age_ijb_fy <- filter_select(
  id = "filter_ijb_age_pop",
  label = "Select Financial Year:",
  group = ~fy,
  sharedData = rates_df_age,
  multiple = FALSE)

# Dropdown (Rate Type)
rates_filter_age_ijb_rate <- filter_select(
  id = "filter_ijb_age_rate",
...
# Layout
bscols(widths = c(1, 4, 4, NA), NULL, rates_filter_age_ijb_fy, rates_filter_age_ijb_rate, NULL)
...
# Dynamic Plot (Age-Stratified Referral Rates by IAA and Rate Type)
plot_demog_bar(
  data = rates_df_age_ijb, 
  x = "geog", 
  y = "rate", 
  demog = "age_grp_2")

```
create_dynamic_table <- function(data, measure, geog_filter, dropdown_col, dropdown_grp, pivot_col, demog)
...
# Dynamic Plot Data (Age-Stratified Referral Rates by Health Board and Rate Type)
rates_df_age_hb <- SharedData$new(
  referral_rates_age %>%
...
# Dropdown (Year)
rates_filter_age_hb_fy <- filter_select(
  id = "filter_hb_age_pop",
  label = "Select Financial Year:",
  group = ~fy,
  sharedData = rates_df_age_hb,
  multiple = FALSE)

# Dropdown (Rate Type)
rates_filter_age_hb_rate <- filter_select(
  id = "filter_hb_age_rate",
  label = "Select Rate Measure:",
  group = ~rate_type,
  sharedData = rates_df_age_hb,
  multiple = FALSE)
...
# Layout
bscols(widths = c(1, 4, 4, NA), NULL, rates_filter_age_hb_fy, rates_filter_age_hb_rate, NULL)
...
# Dynamic Plot (Age-Stratified Referral Rates by Health Board and Rate Type)
plot_demog_bar(
  data = rates_df_age_hb, 
  x = "geog", 
  y = "rate", 
  demog = "age_grp_2")
...
bscols(widths = 12,
       div(style = css(width="97%", height="240px", margin="20px"),
...
simd_totals<-ldp_all_simd %>% group_by(geog,simd) %>% summarise(total_referrals = sum(complete+exempt+fail+ongoing, na.rm = T))
#replaces nas with zeroes for areas that happen to have no referrals in a certain quintile for a financial year,
#but nas remain for those that have zero population in a certain quintile
ldp_all_simd_w_totals<-left_join(ldp_all_simd, simd_totals) %>% 
  mutate(across(complete:ongoing, ~if_else(total_referrals !=0 & is.na(perc_met) & is.na(ongoing), 0, .)))

ijb_ldp_simd_table_data <- ijb_table_ldp(ldp_all_simd_w_totals, simd)
...
```{r table-hb-simd-ldp}
hb_ldp_simd_table_data <- hb_table_ldp(ldp_all_simd_w_totals, simd)
