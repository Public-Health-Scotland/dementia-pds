...
ldp_ijb_age <- ldp_all_age %>% filter(!grepl("NHS", geog)) %>%  
  mutate(perc_met = if_else(is.na(perc_met), 0, perc_met))  %>% #this was added to avoid any areas with missing data being placed at the end of the chart
         mutate(age_grp_2_group = "All", .before = age_grp_2)  %>% 
  mutate(key = paste0(fy,age_grp_2_group,geog), .before = geog)
...
ldp_all_age_data <- SharedData$new(ldp_all_age %>% 
  mutate(perc_met = if_else(geog == "Scotland", NA, perc_met)), key = ~geog, group = "ldp_age")
...
ldp_all_sex_data <- SharedData$new(ldp_all_sex %>% 
  mutate(perc_met = if_else(geog == "Scotland", NA, perc_met)), key = ~geog, group = "ldp_sex")
...
ldp_all_simd_data <- SharedData$new(ldp_all_simd %>% 
  mutate(perc_met = if_else(geog == "Scotland", NA, perc_met)), key = ~geog, group = "ldp_simd")
