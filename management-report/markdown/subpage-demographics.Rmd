...
pds_all_sex <- pds_all %>% group_by(geog, fy, sex) %>% filter(sex != "98 Not Specified", sex != "99 Not Known", sex != "Unknown") %>% 
  summarise(total_referrals = sum(referrals), .groups = "drop") %>% 
complete(nesting(geog, fy), sex, fill = list(total_referrals = 0))


#prepare population rate data with age groups

pop_data_sex <- pop_data %>% filter(age_grp == "All", age_grp_2 == "All", sex != "All") %>% select(-age_grp, -age_grp_2)
...
data_filter_sex_prop <- filter_select(id = "trend_filter_sex_prop_hb",
                                    label = "Select Financial Year of Diagnosis:",
                                    group = ~fy,
...
data_filter_sex_prop <- filter_select(id = "trend_filter_sex_prop_ijb",
                                    label = "Select Financial Year of Diagnosis:",
                                    group = ~fy,
...
```{r plot-hb-simd-legend-pop}
 ...
 bscols(widths = 12,
 div(id="popsimdlegendhb", plot_geog_simd_legend(pds_all_simd %>% filter(geog == "Scotland")), style="margin-bottom:-351px"))
...
```{r plot-ijb-simd-legend-pop}
 ...
 bscols(widths = 12,
 div(id="popsimdlegendijb", plot_geog_simd_legend(pds_all_simd %>% filter(geog == "Scotland")), style="margin-bottom:-351px"))
