...
pds_ijb_age <- pds_all_age %>% filter(!grepl("NHS", geog))

pds_hb_age <- pds_all_age %>% filter(grepl("NHS", geog) | geog == "Scotland")
...
pds_all_simd_rates <- left_join(pds_all_simd_cal_year, pop_data_simd) %>% select(-year) %>%
  mutate(pop_rate_10000 = round((total_referrals/pop)*10000, 1)) %>% select(geog, fy, simd, pop_rate_10000)
...
pds_all_simd_rates %<>% complete(nesting(geog, fy), simd, fill = list(pop_rate_10000 = 0))
...
pds_age_data_hb <- SharedData$new(pds_hb_age, key = ~fy, group = "age_prop_hb")
...
<h2 style="margin-left: 60px; margin-top: 0px; margin-bottom: 0px"> Proportion of Referrals by Age Group and Health Board </h2>

```{r plot-prop-age-legend-hb}

bscols(widths = 12,
div(id="ageproplegend", plot_prop_legend(pds_hb_age, measure = age_grp, measure_text = "Age Group: "), style="margin-bottom:-353px"))
...
```{r plot-prop-age-hb}
bscols(widths = 12,
  div(style = css(width="98%", height="520px"),
...
pds_age_data_ijb <- SharedData$new(pds_ijb_age, key = ~fy, group = "age_prop_ijb")
...
<h2 style="margin-left: 60px; margin-top: 0px; margin-bottom: 0px"> Proportion of Referrals by Age Group and Integration Authority Area</h2>

```{r plot-prop-age-legend-ijb}

bscols(widths = 12,
div(id="ageproplegend", plot_prop_legend(pds_ijb_age, measure = age_grp, measure_text = "Age Group: "), style="margin-bottom:-353px"))
...
```{r plot-prop-age-ijb}
bscols(widths = 12,
  div(style = css(width="98%", height="530px"),
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 10px"> Number of people referred to PDS per 10,000 population by SIMD Quintile and Financial Year</h2> 

```{r plot-trend-simd-legend-pop}

bscols(widths = c(1,1,1,NA),NULL,
       div(style = css(width="100px", height="20px"),
plot_trend_simd_line_legend_1(pds_all_simd_rates %>% filter(geog == "Scotland"))),NULL,
      div(style = css(width="100%", height="23px"),
plot_trend_simd_line_legend_2(pds_all_simd_rates %>% filter(geog == "Scotland")))
)
# 
#  bscols(widths = 12,
...
plot_trend_simd_line(pds_pop_simd_data, measure = pop_rate_10000, y = "Rate per 10,000 Population"))
)
...
trend_pop_simd_table_data <- pds_all_simd_rates %>%
  select(geog,fy,simd,pop_rate_10000) %>%
  pivot_wider(names_from = fy, values_from = pop_rate_10000) %>%
             rename(`SIMD Quintile` = simd)


trend_simd_table_pop <- SharedData$new(trend_pop_simd_table_data, key = ~geog, group = "trend_simd_pop")


bscols(widths = 12,
       div(style = css(width="97%", height="235px", margin="20px"),
DT::datatable(trend_simd_table_pop, rownames = FALSE,
              options = list(pageLength = 10,
                             scrollX = TRUE,
                             scrollY = FALSE,
                             dom = "t",
...
pds_hb_simd_rates <- pds_all_simd_rates %>% filter(geog == "Scotland" | grepl("NHS", geog)) %>% 
  complete(nesting(geog, fy), simd, fill = list(pop_rate_10000 = 0))
...
<h2 style="margin-left: 50px; margin-top: 0px; margin-bottom: 0px"> Number of people referred to PDS per 10,000 population by SIMD Quintile and Health Board</h2>
```{r plot-hb-simd-legend-pop}

 bscols(widths = 12,
 div(id="popsimdlegendhb",plot_geog_simd_legend(pds_all_simd), style="margin-bottom:-351px"))
...
plot_geog_simd(pds_pop_simd_data_hb, measure = pop_rate_10000, 
...
<h2 style="margin-left: 50px; margin-top: 0px; margin-bottom: 0px"> Number of people referred to PDS per 10,000 population by SIMD Quintile and Integration Authority Area</h2>
```{r plot-ijb-simd-legend-pop}

 bscols(widths = 12,
 div(id="popsimdlegendijb",plot_geog_simd_legend(pds_pop_simd_data_ijb), style="margin-bottom:-351px"))
...
bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-30px"),
plot_geog_simd(pds_pop_simd_data_ijb, measure = pop_rate_10000, 
...
ijb_pop_simd_table_data <- pds_all_simd_rates %>% filter(!grepl("NHS", geog)) %>% 
...
ldp_simd_table_data <- ldp_all_simd %>%
