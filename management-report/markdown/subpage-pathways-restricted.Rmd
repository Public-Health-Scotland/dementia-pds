...
#function for addding percentage to outcomes tables
...
met_data_totals <- left_join(pds_all_totals %>% filter(fy %in% full_years), met_data) %>% 
...
```{r dropdown-exempt}
exempt_data <- pds_all %>% filter(ldp == "exempt", geog %in% c(restricted_hb, restricted_ijb)) %>% group_by(geog, fy, ldp_full) %>% 
...
exempt_data_totals <- left_join(pds_all_totals %>% filter(geog %in% c(restricted_hb, restricted_ijb)), exempt_data) %>% 
  rename(`Health Board/Integration Authority Area` = geog, 
         `Total number of individuals referred to PDS` = total_referrals,
...
```{r dropdown-no-contact-ijb}
 ...
 no_contact_term_ijb <- wait_data_3 %>% filter(ijb %in% restricted_ijb) %>%  
...
wait_data_totals_ijb <- wait_data %>%filter(ijb %in% restricted_ijb) %>%  filter(sex == "All", simd == "All", ijb != "All" | ijb == "Scotland") %>% select(ijb, fy, total_referrals)

no_contact_12_ijb <- pds_all %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>%  filter(!grepl("NHS", geog), ldp_full == "fail - PDS not started and more than 12 months since diagnosis" | ldp_full == "fail - PDS started more than 12 months after diagnosis") %>% 
...
```{r dropdown-no-contact-hb}
 ...
 no_contact_term_hb <- wait_data_3 %>% filter(health_board %in% restricted_hb) %>% 
    filter(sex == "All", termination_or_transition_reason == "All reasons") %>%
...
wait_data_totals_hb <- wait_data %>% filter(health_board %in% restricted_hb) %>%  filter(sex == "All", simd == "All", ijb == "All" | ijb == "Scotland") %>% select(health_board, fy, total_referrals)

no_contact_12_hb <- pds_all %>% filter(geog %in% c(restricted_hb, restricted_ijb)) %>% filter(geog == "Scotland" | grepl("NHS", geog), ldp_full == "fail - PDS not started and more than 12 months since diagnosis" | ldp_full == "fail - PDS started more than 12 months after diagnosis") %>% 
...
```{r dropdown-term-1}
 term_data_2 <- wait_data_3 %>% filter(ijb %in% c(restricted_hb, restricted_ijb)) %>% 
    filter(sex == "All") %>%
    pivot_wider(names_from = ldp, values_from = c(referrals, median_referral_to_termination)) %>%
    mutate(across(starts_with("referrals"), ~ replace(., is.na(.), 0))) %>%
    mutate(across(starts_with("median"), ~ replace(., is.na(.), "-"))) %>%
...
```{r dropdown-term-2}
 term_data <- wait_data_2 %>% filter(ijb %in% c(restricted_hb, restricted_ijb)) %>% 
    filter(sex == "All") %>% 
