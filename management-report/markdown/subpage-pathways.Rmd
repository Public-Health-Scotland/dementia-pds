...
# Load pathways data
wait_data <- read_rds(get_mi_data_path(
  type = "wait_data", 
  ext = "rds", 
  fy = fy,
  qt = qt,
...
uptake_data <- read_rds(get_mi_data_path(
  type = "uptake_data", 
  ext = "rds", 
  fy = fy, 
  qt = qt,
...
The Trend tab includes a chart and table showing the trend of the median days between diagnosis and contact by financial year for Scotland, Integration Authority Areas and Health Boards. Multiple areas can be selected. 

The Waiting Times Breakdown tab provides a table showing the number of referrals, percentage of referrals allocated and contacted by PDS practitioner, and the median number of days between each stage of the pathway. The figures presented here will be reflective of how local services and systems are set up, e.g. referral date and/or date of allocation to a PDS practitioner may be recorded at the same time as the diagnosis. Due to the quality of referral date recording in the three years 2016/17 to 2018/19, data for these three years is not presented in these tables. Recording quality improved for the years 2019/20 onwards with the introduction of updated data collection guidance.
...
**Please note:** *NHS Shetland did not have a PDS worker in post from 2022/23 Q1 through 2023/24 Q3. This will affect the pathway waiting times for NHS Shetland for 2022/23 and 2023/24. A PDS worker was assigned in 2023/24 Q4 and the PDS service in Shetland has resumed.*
...
median_data_ijb <- wait_data %>% 
      filter(sex == "All") %>% 
     filter(ijb != "All", simd == "All", ijb != "Unknown") %>% 
...
median_data_ijb_shared <- SharedData$new(median_data_ijb, key = ~fy, group = "median_ijb")

data_filter_median <- filter_select(id = "filter_median",
                                    label = "Select Financial Year of Diagnosis:",
                                    group = ~fy,
                                    sharedData = median_data_ijb_shared,
                                    multiple = FALSE
                                   )
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px">Average (median) days from diagnosis to first contact by PDS practitioner; Integration Authority Areas</h2>
```{r plot-median-ijb}

bscols(widths = 12,
  div(style = css(width="98%", height="385px", marginBottom="-30px"),
plot_bar_median(median_data_ijb_shared))
)
...
median_ijb_table_data <- wait_data %>% 
  ...
  complete(ijb, fy, fill = list(total_referrals = 0)) %>% 
  filter(ijb != "All", ijb != "Unknown") %>% 
   mutate(median_diagnosis_to_contact = if_else(is.na(median_diagnosis_to_contact)| median_diagnosis_to_contact < 0, "-", as.character(median_diagnosis_to_contact))) %>% 
  mutate(across(starts_with("perc"), ~ if_else(is.na(.), "-", paste0(.,"%")))) %>%
...
DT::datatable(median_ijb_table, rownames = FALSE, extensions = "FixedColumns",
               caption = htmltools::tags$caption(
                       ...
                       "A dash (-) indicates that due to data quality issues, or lack of data, this measure could not be calculated."),
...
<h2 style="margin-left: 70px; margin-top: 0px; margin-bottom: 0px">Average (median) days from diagnosis to contact by PDS practitioner; Health Boards</h2>
```{r plot-median-hb}
...
```{r plot-wait-trend}
  ...
  wait_plot_trend <- plotly_trend_median(wait_data_trend)
...
```{r table-wait-trend}
wait_data_trend_table <- wait_data_trend_all
wait_data_trend_table$geog<- factor(wait_data_trend_table$geog, levels = unique(pds_all$geog))

wait_data_trend_table %<>% arrange(geog) %>% 
  mutate(median_diagnosis_to_contact = if_else(is.na(median_diagnosis_to_contact)| median_diagnosis_to_contact < 0, "-", as.character(median_diagnosis_to_contact))) %>% 
...
bscols(widths = 12,
       div(style = css(width="97%", height="615px", margin="20px"),
...
wait_data_all <- wait_data %>% 
      filter(sex == "All", simd == "All", !fy %in% c("2016/17", "2017/18", "2018/19")) %>% 
  mutate(ijb = if_else(ijb == "All", health_board, ijb)) %>% 
  rename(geog = ijb) %>% 
...
not_met_data_totals <- left_join(pds_all_totals %>% 
    ...
    mutate(perc_of_total = if_else(total_referrals == 0, "-",
      paste0(round_half_up(100*total/total_referrals,1), "%")),
...
bscols(widths = 12,
       div(style = css(width="97%", height="605px", margin="20px"),
    DT::datatable(not_met_table_data, container = headers_notmet,
                         rownames = FALSE,
                          caption = htmltools::tags$caption(
                          style = 'caption-side: bottom; text-align: left; color:black;',
                          "PDS terminated without contact refers to those with no contact date recorded and a termination date recorded at any time after diagnosis, therefore this may include referrals with a termination date more than 12 months after diagnosis."),
                         options = list(pageLength = 46,
                                        scrollX = FALSE,
                                        scrollY = TRUE,
                                        dom = "t",
...
ongoing_data_totals <- left_join(pds_all_totals %>% 
   ...
   mutate(`% of Total referrals PDS Ongoing` = if_else(total_referrals == 0, "-",
            paste0(round_half_up(100*total/total_referrals,1), "%")),
                                              .after = total_referrals) %>% 
...
exempt_data_totals <- left_join(pds_all_totals %>% 
   ...
   mutate(`% of Total referrals Exempt` =  if_else(total_referrals == 0, "-",
            paste0(round_half_up(100*total/total_referrals,1), "%")),
                                              .after = total_referrals) %>% 
  rename(`Integration Authority Area/Health Board` = geog, 
         `Total referrals` = total_referrals,
 ...
 table_ijb_uptake_data <- left_join(bind_rows(pds_scot_totals, pds_ijb_totals),
  ...
  uptake_data %>% 
      ...
      mutate(across(where(is.numeric), ~ replace(., is.na(.), 0))) %>% rename("geog" = "ijb")
       ...
       mutate(perc_known = if_else(total_referrals == 0, "-",
                paste0(round_half_up(100*Known/total_referrals,1), "%"))) %>%
      select(2,1,4,7,8,9,10,11,12) %>% 
  mutate(perc_declined = if_else(Known == 0 | is.na(Known), "-", paste0(round_half_up(100*Declined/Known,1), "%"))) %>%
  mutate(perc_accepted = if_else(Known == 0 | is.na(Known), "-", paste0(round_half_up((Accepted + `Accepted, but Initially Declined`)/Known*100,1), "%"))) %>%  
          mutate(across(where(is.numeric), ~format(., big.mark = ","))
                    ) %>% 
  ...
  uptake_data %>% 
     ...
     filter(ijb == "All" | ijb == "Scotland") %>% 
      ...
      mutate(across(where(is.numeric), ~ replace(., is.na(.), 0))) %>% rename("geog" = "health_board")
       ...
       mutate(Known = total_referrals - `Not Known`) %>% 
       mutate(perc_known = paste0(round_half_up(100*Known/total_referrals,1), "%")) %>%
      select(2,1,4,7,8,9,10,11,12) %>% 
  mutate(perc_declined = if_else(Known == 0, "-", paste0(round_half_up(100*Declined/Known,1), "%"))) %>%
  mutate(perc_accepted = if_else(Known == 0, "-", paste0(round_half_up((Accepted + `Accepted, but Initially Declined`)/Known*100,1), "%"))) %>%  
          mutate(across(where(is.numeric), ~format(., big.mark = ","))) %>% 
  select(fy, geog, total_referrals, perc_known, perc_accepted, perc_declined) %>% 
  rename("Number of People Referred to PDS" = "total_referrals",
         "% of referrals where Uptake Decision is Recorded" = "perc_known",
...
bscols(widths = 12,
       div(style = css(width="98%", height="625px", margin="15px"),
...
the number and percentage of total referrals whose PDS was ended with no contact date recorded, excluding those exempt from the LDP Standard. 
...
no_contact_term_ijb <- pds_all %>% filter(!grepl("NHS", geog), ldp_full == "fail - PDS terminated before first contact") %>% group_by(fy, geog) %>% summarise(referrals_no_contact_term = sum(referrals)) %>% rename(ijb = geog)
...
no_contact_ijb <- left_join(ijb_totals_12, no_contact_term_ijb) %>% 
  mutate(across(starts_with("referrals"), ~ replace(., is.na(.), 0))) %>%
    mutate(across(starts_with("median"), ~ replace(., is.na(.), "-"))) %>% 
     mutate(perc_no_contact_term = paste0(round_half_up((referrals_no_contact_term/total_referrals)*100,1),"%")) %>% 
   mutate(perc_no_contact_12 = paste0(round_half_up((referrals_no_contact_12/total_referrals)*100,1),"%")) %>% 
  select(fy, ijb,
         total_referrals,
...
bscols(widths = 12,
       div(style = css(width="97%", height="750px", margin="20px"),
     DT::datatable(no_contact_table_data_ijb, rownames = FALSE, caption = htmltools::tags$caption(
                       style = 'caption-side: bottom; text-align: left; color:black;',
                       "Referrals not contacted within 12 months of diagnosis includes records with no contact date and those with a contact date more than 12 months after diagnosis, but does not include those with a termination date.", htmltools::tags$br(), 
                       "Number of referrals whose PDS ended with no contact refers to those with no contact date recorded and a termination date recorded at any time after diagnosis, therefore this may include referrals with a termination date more than 12 months after diagnosis.
"),
              options = list(pageLength = 33,
                             scrollX = FALSE,
                             scrollY = TRUE,
...
no_contact_term_hb <- pds_all %>% filter(geog == "Scotland" | grepl("NHS", geog), ldp_full == "fail - PDS terminated before first contact") %>% group_by(fy, geog) %>% summarise(referrals_no_contact_term = sum(referrals)) %>% rename(health_board = geog)
