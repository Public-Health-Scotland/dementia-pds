...
term_data_contact %<>% mutate(termination_or_transition_reason = if_else(termination_or_transition_reason == "14 All reasons", substring(termination_or_transition_reason, 3), termination_or_transition_reason))
...
```{r dropdown-term-1}
 term_data_2 <- term_data_no_contact %>%
    filter(sex == "All") %>%
    pivot_wider(names_from = ldp, values_from = c(referrals, median_referral_to_termination)) %>%
    mutate(across(starts_with("referrals"), ~ replace(., is.na(.), 0))) %>%
    mutate(across(starts_with("median"), ~ replace(., is.na(.), "-"))) %>%
