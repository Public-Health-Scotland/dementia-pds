...
met_data <- pds_all %>% filter(ldp == "complete") %>% group_by(geog, fy, ldp_full) %>% 
  summarise(referrals = sum(referrals)) %>% mutate(ldp_full = substring(ldp_full, 12)) %>% 
...
not_met_data <- pds_all %>% filter(ldp == "fail") %>% group_by(geog, fy, ldp_full) %>% 
  summarise(referrals = sum(referrals)) %>% mutate(ldp_full = substring(ldp_full, 7)) %>% 
  pivot_wider(names_from = ldp_full, values_from = referrals) %>%
  rename(`Health Board/Integration Authority Area` = geog) %>% 
   mutate(`Total Number of individuals` = rowSums(across(where(is.numeric))), .after = fy)
...
ongoing_data <- pds_all %>% filter(ldp == "ongoing") %>% group_by(geog, fy, ldp_full) %>% 
  summarise(referrals = sum(referrals)) %>% mutate(ldp_full = substring(ldp_full, 11)) %>% 
  pivot_wider(names_from = ldp_full, values_from = referrals) %>%
  rename(`Health Board/Integration Authority Area` = geog) %>% 
   mutate(`Total Number of individuals` = rowSums(across(where(is.numeric))), .after = fy)
...
exempt_data <- pds_all %>% filter(ldp == "exempt") %>% group_by(geog, fy, ldp_full) %>% 
  summarise(referrals = sum(referrals)) %>% mutate(ldp_full = substring(ldp_full, 10)) %>% 
  pivot_wider(names_from = ldp_full, values_from = referrals) %>%
  rename(`Health Board/Integration Authority Area` = geog) %>% 
   mutate(`Total Number of individuals` = rowSums(across(where(is.numeric))), .after = fy)
