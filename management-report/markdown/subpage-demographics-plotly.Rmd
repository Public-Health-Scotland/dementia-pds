...
ijb_pop_age_table_data <- bind_rows(
  ...
  pivot_wider(names_from =age_grp_2, values_from = pop_rate_10000) %>% 
    mutate(age_2_group = "65+", .before = fy),
  
  pds_all_age_2_rates %>% 
  select(geog,fy,age_grp_2,pop_rate_10000) %>% filter(!grepl("NHS", geog)) %>% 
     mutate(pop_rate_10000 = as.character(pop_rate_10000)) %>%
  pivot_wider(names_from =age_grp_2, values_from = pop_rate_10000) %>% 
    mutate(age_2_group = "64 and Under", .before = fy)
) %>% 
...
bscols(widths = 12,
       div(style = css(width="97%", height="320px", margin="20px"),
...
hb_pop_age_table_data <- bind_rows(
  ...
  pds_all_age_2_rates %>% 
  select(geog,fy,age_grp_2,pop_rate_10000) %>% filter(geog == "Scotland" | grepl("NHS", geog)) %>%
     mutate(pop_rate_10000 = as.character(pop_rate_10000)) %>%
  pivot_wider(names_from =age_grp_2, values_from = pop_rate_10000) %>% 
    mutate(age_2_group = "64 and Under", .before = fy)
) %>% 
...
bscols(widths = 12,
  div(style = css(width="98%", height="355px", marginBottom="-30px"),
plot_ly(pds_pop_simd_data_ijb, x = ~geog, y = ~pop_rate_10000, meta = ~fy, hovertext = ~simd, type = 'bar', color = ~simd, colors = simd_colours, 
  ...
  layout(showlegend = FALSE, margin = list(l = -5, b = 10, t = 40),
...
bscols(widths = 12,
       div(style = css(width="97%", height="325px", margin="20px"),
