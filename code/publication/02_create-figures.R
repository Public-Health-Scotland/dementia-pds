...
################################################################################.
### 1 - Load environment file, functions and themes----
################################################################################.
...
c1 <- c1_data %>%
  ...
  geom_text(
    aes(x = perc, y = health_board, label = perc_formatted),
...
c3 <- c3_data %>%
  ...
  geom_bar(stat = "identity", width = 0.8) +
  geom_text(
    aes(x = perc, y = ijb, label = perc_formatted),
    nudge_x = -5, colour = "white", size = 3) +
  theme_dementia_pub() +
  scale_fill_manual(values = c("#3F3685", "#9B4393")) +
  theme(panel.grid.major.y = element_blank()) +
...
c4_data <- basefile %>%
  ...
  #   age_grp == '59 and Under' ~ pub_referrals,  
...
c4 <- c4_data %>% 
  ...
  geom_text(
    aes(label = paste0(format(round_half_up(perc, 1), nsmall = 1), "%")), 
    vjust = -0.5,
    size = 3) +
  theme_dementia_pub() +
  scale_y_continuous(
...
c5_data <- basefile %>% 
  ...
  #     age_grp == 'Unknown' ~ pub_referrals),
  #   numerator = case_when(
  #     age_grp == '59 and Under' ~ pub_numerator,
  ...
  #     age_grp == '59 and Under' ~ pub_denominator,
...
c6_data <- basefile %>%
  ...
  # mutate(referrals = case_when(
  #   simd == '1 - Most Deprived' ~ pub_referrals,
...
c7_data <- basefile %>%
  ...
  #     simd == 'Unknown' ~ pub_referrals),
  #   numerator = case_when(
  #     simd == '1 - Most Deprived' ~ pub_numerator,
  ...
  #   denominator = case_when(
  #     simd == '1 - Most Deprived' ~ pub_denominator,
...
trend_year <- paste0(
  as.numeric(substr(max(fy_in_pub),1,4)) + 1, "/", 
  as.numeric(substr(max(fy_in_pub),6,7)) + 1
)

c8_data <- read_rds(get_mi_data_path(
  type = "final_data", 
  ext = "rds", 
  fy = fy,
  qt = qt,
...
c9_data_pds <- read_rds(get_mi_data_path(
  type = "final_data", 
  ext = "rds", 
  fy = fy,
  qt = qt,
...
# Read in pop_data
pop_data_trends <- read_rds(get_pop_lookup_path()) %>%
  filter(
    geog == "Scotland", 
    age_grp != "All", 
...
c9_data <- left_join(c9_data_pds, pop_data_trends) %>% 
  select(geog, fy, annual_referrals, pop_est) %>% 
...
c9 <- c9_data %>%
  ...
  geom_line(colour = "#3F3685", linewidth = 1.3) +
  # geom_text(
  #   aes(label = format(annual_referrals, big.mark = ",")), 
  #   vjust = 1.5,
  #   size = 3) +
  theme_dementia_pub() +
  scale_y_continuous(
...
# Read in data
c12_data <- read_rds(get_mi_data_path(
  type = "wait_data", 
  ext = "rds", 
  fy = fy,
  qt = qt,
...
c12 <- c12_data %>% 
  # SIMD and sex breakdowns are not included in publication
  ...
  geom_bar(stat = "identity", width = 0.8) +
  geom_text(
    aes(x = median_diagnosis_to_contact, y = health_board, label = median_diagnosis_to_contact),
...
# Read in data
c13_data <- read_rds(get_mi_data_path(
  type = "wait_data", 
  ext = "rds", 
  fy = fy,
  qt = qt,
...
# Read in data
c14_data <- read_rds(get_mi_data_path(
  type = "wait_data", 
  ext = "rds", 
  fy = fy, 
  qt = qt,
