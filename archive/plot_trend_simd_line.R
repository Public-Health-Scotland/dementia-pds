...
plot_trend_simd_line <- function(data, measure, y = NULL){
  
  plot <-  ggplot(data) +
    
    geom_point(aes(x = fy, y = {{measure}}, group = simd, colour = simd,
                   text = paste0(geog, "<br>",
    ...
    geom_line(aes(x = fy, y = {{measure}}, group = simd, colour = simd))+
    
    geom_point(aes(x = fy, y = scot_pop_rate_10000, group = simd, colour = simd,
    ...
    geom_line(aes(x = fy, y = scot_pop_rate_10000, group = simd, colour = simd),
  linetype = 2) +
...
plot_trend_simd_line_legend_1 <- function(data) {
...
plot_trend_simd_line_legend_2 <- function(data) {
