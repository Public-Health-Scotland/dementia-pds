...
inc_trend_comm <-
  ...
  mutate(
    perc_change = last(perc) - perc,
    perc_dir  = case_when(
           perc_change >= 1 ~ "increased",
           perc_change <= -1 ~ "decreased",
           TRUE ~ "similar to"
         ),
    perc_change = format_perc(abs(perc_change), 0)
  )

# Commentary for incidence trend
inc_trend_text <- glue(
  "The {latest_fy} Scotland rate has {inc_trend_comm %>% ",
  "filter(row_number() == n() - 1) %>% pull(perc_dir)} by {inc_trend_comm %>% ",
  "filter(row_number() == n() - 1) %>% pull(perc_change)} from {inc_trend_comm %>% ",
  "filter(row_number() == n() - 1) %>% pull(fy)} but {inc_trend_comm %>% ",
  "filter(row_number() == n() - 2) %>% pull(perc_dir)} by {inc_trend_comm %>% ",
  "filter(row_number() == n() - 2) %>% pull(perc_change)} from {inc_trend_comm %>% ",
...
hb <-
  ...
  adorn_totals(name = "Scotland") %>%
  mutate(
    perc = format_perc(numerator / referrals * 100)
  ) %>%
...
hb_trend <-
  ...
  group_modify(~ adorn_totals(., name = "Scotland")) %>%
  ungroup() %>%
  mutate(perc = numerator / referrals * 100)

# Data for HB trend table
hb_trend_table <-
  hb_trend %>%
  mutate(perc = format_perc(perc)) %>%
  select(-c(referrals:numerator)) %>%
  pivot_wider(names_from = fy, values_from = perc)
...
trend_comm <-
  ...
  mutate(
    perc_change = last(perc) - perc,
    perc_dir  = case_when(
           perc_change > 0 ~ "increased",
           perc_change < 0 ~ "decreased",
           perc_change == 0 ~ "stayed the same"
         )
  ) %>%
  mutate(across(perc:perc_change, ~ format_perc(abs(.))))

# Commentary for Scotland trend
scot_trend_text <- glue(
  "The Scotland rate {trend_comm %>% filter(row_number() == n() - 1) %>% ",
  "pull(perc_dir)} to {trend_comm %>% filter(row_number() == n()) %>% ",
  "pull(perc)} in {trend_comm %>% filter(row_number() == n()) %>% ",
  "pull(fy)} from {trend_comm %>% filter(row_number() == n() - 1) %>% ",
  "pull(perc)} in {trend_comm %>% filter(row_number() == n() - 1) %>% ",
  "pull(fy)}, but showed a {trend_comm %>% filter(row_number() == n() - 2) ",
  "%>% pull(perc_dir) %>% str_remove('d$')} when compared to {trend_comm %>% ",
  "filter(row_number() == n() - 2) %>% pull(fy)} ({trend_comm %>% ",
  "filter(row_number() == n() - 2) %>% pull(perc)})."
)

# Data for HB trend commentary
hb_trend_comm <-
  hb_trend %>%
  filter(health_board != "Scotland" & fy %in% c(latest_fy, revised_fy)) %>%
  group_by(health_board) %>%
  arrange(fy) %>%
  summarise(diff = diff(perc)) %>%
...
dir <- 
  case_when(
  (hb_trend_comm / 14 ) == 0.5 ~ "half",
  (hb_trend_comm / 14 ) > 0.5 ~ "more than half",
  (hb_trend_comm / 14 ) < 0.5 ~ "less than half"
)

# Commentary for HB trend
hb_trend_text <- glue(
  "When comparing {revised_fy} and {latest_fy} figures, {dir} of the NHS ",
  "Boards showed a higher rate in {latest_fy} than in {revised_fy}."
)
...
ijb_trend_table <-
  ...
  group_modify(~ adorn_totals(., name = "Scotland")) %>%
  ungroup() %>%
  mutate(perc = format_perc(numerator / referrals * 100)) %>%
  select(-c(referrals:numerator)) %>%
  pivot_wider(names_from = fy, values_from = perc)
...
metadata <- tribble(
  ...
  "{str_sub(fy_in_pub[1], 1, 4)} to 31 March {str_sub(latest_fy, 1, 4)}. \n",
  ...
  "published prior to 2016/17.  This is due to a combination of the ",
  ...
  glue("Data submissions are received for all NHS Boards and include data ",
       "for all Integration Joint Boards. \n *Add note here re COVID-19*"),
  "Comparability",
  glue("Information presented in this publication is not ",
       "comparable to dementia post-diagnostic support LDP standard ",
       "information published prior to 2016/17.  This is due to a combination ",
       "of the change in definitions, improvements to data quality and ",
       "refinements to the method used to calculate the standard. \n ",
       ...
       "care & have varying resource issues."),
  "Accessibility",
  glue("It is the policy of Public Health Scotland (PHS) to make its web ",
       ...
       "consideration throught the report development and all figures ",
...
<Div custom-style = "TableChart title">Table 2: Trend - Percentage of estimated number of people newly diagnosed with dementia referred for post-diagnostic support (PDS) by NHS Board</Div>
