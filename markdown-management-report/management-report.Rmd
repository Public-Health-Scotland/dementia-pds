...
# Load functions
purrr::map(dir(here::here("functions"), full.names = TRUE), source)
...
for(y in years){
  ...
  out <- c(out, knitr::knit_child('subpage-scotland.Rmd', envir = subpage_env))
...
for (hb in health_boards) {
  ...
  # Knit subpage.RMD using the subpage_env and add result to out vector
  out <- c(out, knitr::knit_child('subpage-hb.Rmd', envir = subpage_env))
