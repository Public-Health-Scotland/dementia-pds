...
output$intro_page_ui <-  renderUI({
  div(
      ...
      column(
        ...
        fluidRow(
          ...
          p("This is the Public Health Scotland Dementia Post-Diagnostic Support (PDS) dashboard. 
            ...
            a("here.", href = paste0("https://publichealthscotland.scot/publications/dementia-post-diagnostic-support/dementia-post-diagnostic-support-local-delivery-plan-standard-figures-to-", str_replace(latest_fy, "/","")), target="_blank")),
          p("This dashboard contains pages providing information on the following areas:",
            tags$ul(tags$li("Total number of referrals to PDS and rates per 10,000 population"),
                    ...
                    tags$li("Demographics (Sex, Age and Deprivation)"),
    ...
    conditionalPanel(
      condition = 'input.home_select == "glossary"',
      fluidRow(column(
        h3(strong("Glossary")),
        ...
        p(strong("Post-Diagnostic Support (PDS):"),"Support commissioned by the Scottish Government that everyone newly diagnosed with dementia is entitled
          ...
          a("Alzheimer Scotland", href ="https://www.alzscot.org/about-us/care/post-diagnostic-support/", target="_blank"), " website."),
    ...
    conditionalPanel(
      condition = 'input.home_select == "access"',
      fluidRow(column(
...
over_65_and_total <- left_join(
  scot_65_plus_total <- data_age %>% 
    filter(type != "59 and Under", type != "60 to 64") %>% 
    group_by(geog = "Scotland") %>% 
    summarise(over_65_referrals = sum(referrals)),
  scot_overall_total <- annual_table_data %>% 
    group_by(geog = "Scotland") %>%  
    filter(ijb == "Scotland", ldp == "total") %>% 
